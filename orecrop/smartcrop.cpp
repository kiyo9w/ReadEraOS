...
static float GetBottomCropBound(const uint8_t *pixels, int width, int height, float ignoreZoneLeft, float ignoreZoneRight )
{
    ...
    for (y = height - h_line_size; y > height - pageLimit_colontitle_v; y -= h_line_size)
    {
...
static bool isLineBlack(const uint8_t *pixels, int w, int h, int y)
{
    unsigned long int lum = 0;
    ...
    unsigned long int avg_lum = lum / w;
    return (avg_lum < 255 * 0.5); // less than 50% of pixels in line are white
}

static float getTopColontitleBound(const uint8_t *pixels, int width, int height)
{
    for (int y = 0; y < pageLimit_colontitle_v; y++)
    {
        bool white = isLineBlack(pixels, width, height, y);
        //LE("GetTopCropBound y = %d white = %d cnt = %d",y,white,white_count);
        if(white)
        {
...
static float getBottomColontitleBound(const uint8_t *pixels, int width, int height)
{
    for (int y = height - 1; y >= height - pageLimit_colontitle_v; y--)
    {
        bool white = isLineBlack(pixels, width, height, y);
        //LE("GetTopCropBound y = %d white = %d cnt = %d",y,white,white_count);
        if(white)
...
void calcBitmapColontitleBounds(float *crop, uint8_t *pixels, int w, int h, float slice_l, float slice_t, float slice_r, float slice_b)
{
    //LE("calcBitmapColontitleBounds w = %d, h = %d ,[%f %f %f %f]",w,h,slice_l,slice_t,slice_r,slice_b);
    pageLimit_colontitle_v = ceil(h * 0.25);
    v_line_margin = ceil(w * 0.05);
    h_line_margin = ceil(h * 0.05);
    //LE("calcBitmapColontitleBounds pageLimit_v = %d",pageLimit_v);

    int size = w * h * 4;
    auto posterized_pixels = (unsigned char *) malloc(size);
    if (posterized_pixels == NULL)
    {
        //LE("calcBitmapColontitleBounds: failed to allocate %d bytes for inverted pixels", size);
        return;
    }
    ...
    for (int y = 0; y < h; y++)
    {
        for (int x = 0; x < w; x++)
        {
            ...
            if ( (x <= w * 0.01) ||  (x >= w * 0.99) || (y <= h/100) || y >= h *0.99)
            {
                posterized_pixels[i + 0] = 255;
                posterized_pixels[i + 1] = 255;
                posterized_pixels[i + 2] = 255;
                posterized_pixels[i + 3] = 255;
                continue;
            }

            int currlum = ceil((pixels[i + 0] + pixels[i + 1] + pixels[i + 2]) / 3);

            int val = (currlum < 128) ? 0 : 255;
            posterized_pixels[i + 0] = val;
            posterized_pixels[i + 1] = val;
            posterized_pixels[i + 2] = val;
            posterized_pixels[i + 3] = 255;
        }
    }

    crop[0] = 0.0f;
    crop[1] = getTopColontitleBound(posterized_pixels, w, h);
    crop[2] = 1.0f;
    crop[3] = getBottomColontitleBound(posterized_pixels, w, h);
    ...
    crop[0] = crop[0] * (slice_r - slice_l) + slice_l; //l
    crop[1] = crop[1] * (slice_b - slice_t) + slice_t; //t
    crop[2] = crop[2] * (slice_r - slice_l) + slice_l; //r
    crop[3] = crop[3] * (slice_b - slice_t) + slice_t; //b
    free(posterized_pixels);
    //LE("calcBitmapColontitleBounds result = [%f %f %f %f]",crop[0],crop[1],crop[2],crop[3]);
}
