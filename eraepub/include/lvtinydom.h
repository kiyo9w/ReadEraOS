...
public:
    ...
    /// recursively looks for child node with certain nodename, returns true if found one
    bool hasChild(const char *name );
    ldomNode * getChild(const char *name );
...
public:
    ...
    int countAllDescendants() const {
        int count = getChildCount();
        for (int i = 0; i < getChildCount() ; i++)
        {
            count += getChild(i)->countAllDescendants();
        }
        return count;
    }
