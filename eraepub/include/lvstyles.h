...
void printStyleHashFrags(css_style_ref_t style)
{
    ...
    LE("====================================");
    LE(" display             = %x",(lUInt32) rec->display);
    LE(" white_space         = %x",(lUInt32) rec->white_space);
    LE(" text_align          = %x",(lUInt32) rec->text_align);
    LE(" text_align_last     = %x",(lUInt32) rec->text_align_last);
    LE(" text_decoration     = %x",(lUInt32) rec->text_decoration);
    LE(" hyphenate           = %x",(lUInt32) rec->hyphenate);
    LE(" list_style_type     = %x",(lUInt32) rec->list_style_type);
    LE(" letter_spacing      = %x",(lUInt32) rec->letter_spacing.pack());
    LE(" list_style_position = %x",(lUInt32) rec->list_style_position);
    LE(" page_break_before   = %x",(lUInt32) (rec->page_break_before | (rec->page_break_before << 4) | (rec->page_break_before << 8)));
    LE(" vertical_align      = %x",(lUInt32) rec->vertical_align);
    LE(" font_size.type      = %x",(lUInt32) rec->font_size.type);
    LE(" font_size.value     = %x",(lUInt32) rec->font_size.value);
    LE(" font_style          = %x",(lUInt32) rec->font_style);
    LE(" font_weight         = %x",(lUInt32) rec->font_weight);
    LE(" line_height         = %x",(lUInt32) rec->line_height.pack());
    LE(" color               = %x",(lUInt32) rec->color.pack());
    LE(" background_color    = %x",(lUInt32) rec->background_color.pack());
    LE(" width               = %x",(lUInt32) rec->width.pack());
    LE(" height              = %x",(lUInt32) rec->height.pack());
    LE(" text_indent         = %x",(lUInt32) rec->text_indent.pack());
    LE(" margin              =[%x][%x][%x][%x]",(lUInt32) rec->margin[0].pack(),(lUInt32) rec->margin[1].pack(),(lUInt32) rec->margin[2].pack(),(lUInt32) rec->margin[3].pack());
    LE(" padding             =[%x][%x][%x][%x]",(lUInt32) rec->padding[0].pack(),(lUInt32) rec->padding[1].pack(),(lUInt32) rec->padding[2].pack(),(lUInt32) rec->padding[3].pack());
    LE(" font_family         = %x",(lUInt32) rec->font_family);
    LE(" font_name.getHash() = %x",(lUInt32) rec->font_name.getHash());
    LE("====================================");
};
*/
