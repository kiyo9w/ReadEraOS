...
public:
    ...
    std::string getXpointer()
    {
        ...
        return std::string(LCSTR(word_.getStartXPointer().toString()));
    }

    bool checkXpointer(std::string xp)
    {
        if(!xpointer_.empty())
        {
            return xpointer_ == xp;
        }
        //auto end   = std::string(LCSTR(word_.getEndXPointer().toString()));
        //if(xp == end)
        //{
        //    return true;
        //}
        auto start = std::string(LCSTR(word_.getStartXPointer().toString()));
...
public:
    PageHitboxesCache(){};
    PageHitboxesCache(int page, LVArray<Hitbox> Hitboxes){
        page_ = page;
...
class PageHitboxesCacheManager
{
private:
    std::vector<PageHitboxesCache> array;
    const int maxsize = 10;

public:
    PageHitboxesCacheManager()
    {
    ...
    void addItem(PageHitboxesCache cache)
    {
        if(cache.isEmpty())
        {
            return;
        ...
        if(!pageExists(cache.getPage()))
        {
            array.emplace_back(cache);
        }
    ...
    LVArray<Hitbox> findPageHitboxes(int pagenum)
    {
        if (array.empty())
        {
            return LVArray<Hitbox>();
        }
        for (int i = array.size() - 1; i >= 0; i--)
        {
            if (array.at(i).getPage() == pagenum)
            {
                return array.at(i).getArray();
            }
        }
        return LVArray<Hitbox>();
    };
...
private:
    ...
    bool NeedCheckImage();
    lString16 cachedPageText = lString16::empty_str;
    int cachedPage = -2;
public:
    std::vector<std::pair<std::wstring,std::wstring>> searchDictionary;      //orig + prepared
    std::vector<std::wstring> searchDictionaryCJK;                           //just orig
    std::vector<std::pair<std::wstring,std::wstring>> searchDictionaryRTL;   //orig + prepared
    std::vector<std::pair<std::wstring,std::wstring>> searchDictionaryIndic; //orig + prepared
    ...
    PageHitboxesCacheManager hitboxesCacheManager;
...
//    bool DrawImageTo(LVDrawBuf* buf, LVImageSourceRef img, int x, int y, int dx, int dy);
    ...
    std::vector<lString16> getAllImagesXpaths();
    ...
    LVArray<TextRect> GetSearchHitboxes(LVArray<ldomWord> words_list, lString16 query, bool wordBeginFlag = false);

    LVArray<Hitbox> SearchForTextHitboxes(int page, lString16 query, bool wordBeginFlag = false);
    ...
    std::vector<Hitbox> unionRects(std::vector<Hitbox> rects);
    ...
    lvRect FillInbetweenSpace(lvRect input, int i, LVArray<TextRect> *word_chars);

    int doRoughXpathSearch(const lString16& xpath, const LVArray<Hitbox>& hitboxes);
};
