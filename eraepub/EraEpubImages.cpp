...
void CreBridge::processImagesXpaths(CmdRequest &request, CmdResponse &response)
{
    ...
    std::map<lString16,int> map;

    std::vector<lString16> images_array = doc_view_->getAllImagesXpaths();

    for (auto xp : images_array)
    {
        if(map.find(xp) != map.end())
        {
            continue;
        }
        else
        {
            map.insert(std::make_pair(xp,1));
            //LD("processImagesXpaths: [%s]",LCSTR(xp));
            responseAddString(response,xp);
        }
    }
...
void CreBridge::processImageByXpath(CmdRequest &request, CmdResponse &response)
{
    ...
    if (bm.isNull())
    {
        LE("processImageByXpath bad xpath bm.isNull()");
        response.result = RES_BAD_REQ_DATA;
        return;
    }
...
void CreBridge::processImageHitbox(CmdRequest &request, CmdResponse &response)
{
    ...
    for (int i = 0; i < hitboxes.length(); i++)
    {
        ...
        lString16 xpath = currHitbox.getNode()->getXPath();
