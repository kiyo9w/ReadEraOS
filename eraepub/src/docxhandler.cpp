    ...
    if (!container_stream.isNull())
    {
        ...
        if (doc)
        {
            ...
            for (int i = 1; i < 300; i++)
            {
                ...
                {
                    auto img = new DocxLink;
                    img->id_=id;
                    img->type_=type;
                    img->target_=target;
                    img->targetmode_=targetmode;
                    linkslist.add(img);
                }
            }
            delete doc;
...
int DocxGetStyleOutlineLvl(ldomNode * node)
{
    if(node->isNodeName("outlineLvl"))
    {
        lString16 val = node->getAttributeValue("val");
        if(val!=lString16::empty_str)
        {
    ...
    {
        ldomNode * child = node->getChildNode(i);
        int val = DocxGetStyleOutlineLvl(child);
        if(val != -1)
        {
            return val;
        }
        ...
        if (doc)
        {
            ...
            while (i<500)
            {
                ...
                if (type == "paragraph")
                {
                    ...
                    int outline_lvl = DocxGetStyleOutlineLvl(item);
                    //LE("name = [%s], lvl = %d, size = %d",LCSTR(name),outline_lvl, size);
                    if (size != -1)
                    {
                        bool def = (isDefault == "1" && DocxStyles.default_size_ == -1);
                        if(def)
                        {
                            DocxStyles.default_size_ = size;
                        }
                        auto *style = new DocxStyle(type, size, styleId, def, name, outline_lvl);
                        DocxStyles.add(style);
                    }
                    else if (outline_lvl != -1)
                    {
                        bool def = (isDefault == "1" && DocxStyles.default_size_ == -1);
                        auto *style = new DocxStyle(type, -1, styleId, def, name, outline_lvl);
                        DocxStyles.add(style);
                    }
                }
                i++;
...
bool DocxStyles::updateMiniMax()
{
    for (int i = 0; i < this->length(); i++)
    {
        ...
        if(curr->fontSize_>max_)
            max_ = curr->fontSize_;
        if(curr->fontSize_<min_ && curr->fontSize_ != -1)
            min_ = curr->fontSize_;
    }
...
void DocxStyles::checkForHeaders()
{
    for (int i = 0; i < this->length(); i++)
    {
        ...
        if(curr->outline_lvl_ != -1)
        {
            switch (curr->outline_lvl_)
            {
                case 0: h1id_ = (h1isset)? h1id_: curr->styleId_; h1isset = true; break;
                case 1: h2id_ = (h2isset)? h2id_: curr->styleId_; h2isset = true; break;
                case 2: h3id_ = (h3isset)? h3id_: curr->styleId_; h3isset = true; break;
                case 3: h4id_ = (h4isset)? h4id_: curr->styleId_; h4isset = true; break;
                case 4: h5id_ = (h5isset)? h5id_: curr->styleId_; h5isset = true; break;
                case 5: h6id_ = (h6isset)? h6id_: curr->styleId_; h6isset = true; break;
                default:
                    LE("Error: Unknown header style found! [%s]",LCSTR(curr_name));
                    break;
            }
            continue;
        }
        ...
        {
            //LE("curr name = [%s]",LCSTR(curr_name));
            lString16Collection namefrags;
            namefrags.parse(curr_name,' ', false);
            for (int j = 0; j < namefrags.length(); j++)
            {
                lString16 namefrag  = namefrags.at(j);
                if(namefrag.DigitsOnly())
                {
                    int headnum = namefrag.atoi();
                    switch (headnum)
                    {
                        case 1: h1id_ = (h1isset)? h1id_: curr->styleId_; h1isset = true; break;
                        case 2: h2id_ = (h2isset)? h2id_: curr->styleId_; h2isset = true; break;
                        case 3: h3id_ = (h3isset)? h3id_: curr->styleId_; h3isset = true; break;
                        case 4: h4id_ = (h4isset)? h4id_: curr->styleId_; h4isset = true; break;
                        case 5: h5id_ = (h5isset)? h5id_: curr->styleId_; h5isset = true; break;
                        case 6: h6id_ = (h6isset)? h6id_: curr->styleId_; h6isset = true; break;
                        default:
                            LE("Error: Unknown header style found! [%s]",LCSTR(curr_name));
                            break;
                    }
...
bool ImportDocxDocument(LVStreamRef stream, CrDom *m_doc, bool firstpage_thumb)
{
    ...
    if (arc.isNull())
    {
        LE("This Docx is corrupted: not a ZIP archive");
    ...
    {
        LE("No main document file found! This is either not a docx file, or the file is corrupted!");
        return false;
    }
    ...
    if (decryptor->open())
    {
        LD("DOCX: encrypted items detected");
    }
...
*/
 ...
 /*
 LE("styles.legnth = %d",docxStyles.length());
    LE("min = %d",docxStyles.min_);
    LE("def = %d",docxStyles.default_size_);
    LE("max = %d",docxStyles.max_);
    for (int i = 0; i < docxStyles.length(); i++)
    {
        DocxStyle* style = docxStyles.get(i);
        LE("style = [%s] [%s] [%d] [%d]",LCSTR(style->type_),LCSTR(style->styleId_),style->fontSize_,style->isDefault_?1:0);
    }
*/
    ...
    if (!stream2.isNull())
    {
        ...
        {
            LE("Unable to parse docx file at [%s]", LCSTR(rootfilePath));
        }
    }
    ...
    if(!footnotesFilePath.empty())
    {
        ...
        if (!stream3.isNull())
        {
            ...
            {
                LE("Unable to parse footnotes file at [%s]", LCSTR(footnotesFilePath));
            }
        }
        else
        {
            LE("Failed opening footnotes file at [%s]",LCSTR(footnotesFilePath));
        }
    }
    else
    {
        LV("No footnotes found in docx package.");
    }
    writer.OnTagClose(L"", L"body");
    //footnotes
    if(!LinksList.empty())
    {
        ...
        //{
        //	LE("LinksList %d = %s = %s",i,LCSTR(LinksList.get(i).id_),LCSTR(LinksList.get(i).href_));
        //}
    ...
    CleanupDocxHeaders(m_doc->getRootNode());
    m_doc->getToc()->clear();
    GetTOC(m_doc,m_doc->getToc());

    if (m_doc->getToc()->getChildCount() == 0)
    {
        LI("Docx bookmarkstart-bookmarkend TOC search");
        GetDocxBookmarkTOC(m_doc, m_doc->getToc());
    }
    if (m_doc->getToc()->getChildCount() == 0)
    {
