...
void LVRendPageContext::enterFootNote( lString16 id )
{
    if ( !page_list )
        return;
    //LV("enterFootNote( %s )", LCSTR(id) );
    if ( curr_note != NULL ) {
        LE("Nested entering note" );
        return;
    }
...
void LVRendPageContext::leaveFootNote()
{
    ...
    //LV("leaveFootNote()" );
    if ( !curr_note ) {
        LE("leaveFootNote() w/o current note set");
    }
    curr_note = NULL;
}
...
void LVRendPageContext::AddLine( int starty, int endy, int flags )
{
    ...
    lines.add( line );
    if ( curr_note != NULL ) {
        //LV("adding line to note (%d)", line->start);
...
public:
    ...
    void StartPage( const LVRendLineInfo * line )
    {
        ...
        // if ( CRLog::isTraceEnabled() )
            LV("StartPage(%d)", line ? line->start : -111111111);
#endif
        last = pagestart = line;
        pageend = NULL;
        next = NULL;
    ...
    void AddToList()
    {
        ...
        //if ( CRLog::isTraceEnabled() )
        {
            if ( pagestart && pageend )
                LV("AddToList(%d, %d) footnotes: %d  pageHeight=%d", pagestart->start, pageend->start+pageend->height, footnotes.length(), h);
            else
                LV("AddToList(Only footnote: %d) footnotes: %d  pageHeight=%d", lastpageend, footnotes.length(), h);
        }
#endif
...
//                }
    ...
    {
#ifdef DEBUG_FOOTNOTES
        LV( "StartFootNote(%d)", note->getLines().length() );
#endif
    ...
    void AddFootnoteFragmentToList()
    {
        ...
        if ( footend==NULL )
            footend = footstart;
        //LV("AddFootnoteFragmentToList(%d, %d)", footstart->getStart(), footend->getEnd() );
        int h = footend->getEnd() - footstart->getStart(); // currentFootnoteHeight();
        if ( h>0 && h<page_h ) {
            footheight += h;
#ifdef DEBUG_FOOTNOTES
            LV("AddFootnoteFragmentToList(%d, %d)", footstart->getStart(), h);
#endif
...
#ifdef DEBUG_FOOTNOTES
        LV("Add footnote line %d  footheight=%d  h=%d  dh=%d  page_h=%d", line->start, footheight, h, dh, page_h);
#endif
        if ( h + dh > page_h ) {
#ifdef DEBUG_FOOTNOTES
            LV("No current page space for this line, %s", (footstart?"footstart is not null":"footstart is null"));
#endif
            if ( footstart==NULL ) {
                //LV("Starting new footnote fragment");
...
void LVRendPageContext::split()
{
    ...
    for ( int lindex=0; lindex<lineCount; lindex++ ) {
        ...
        if ( line->getLinks() ) {
            ...
            //    LV("LVRendPageContext::split() line %d: found %d links", lindex, line->getLinks()->length() );
           // }
