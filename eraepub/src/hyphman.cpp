...
bool HyphMan::activateDictionaryFromStream(LVStreamRef stream)
{
    if ( stream.isNull() )
        return false;
    LV("remove old hyphenation method");
    if ( HyphMan::_method != &NO_HYPH && HyphMan::_method != &ALGO_HYPH && HyphMan::_method ) {
        delete HyphMan::_method;
        HyphMan::_method = &NO_HYPH;
    }
    LV("creating new TexHyph method");
    TexHyph* method = new TexHyph();
    if (!method->load(stream)) {
		LE("HyphMan::activateDictionaryFromStream: error" );
        delete method;
        return false;
    }
    LD("Dictionary is loaded successfully. Activating.");
...
bool HyphDictionary::activate()
{
	...
	if (getType() == HDT_ALGORITHM) {
		LV("Turn on algorythmic hyphenation");
		...
		LV("Selecting hyphenation dictionary %s", UnicodeToUtf8(_filename).c_str() );
		LVStreamRef stream = LVOpenFileStream(getFilename().c_str(), LVOM_READ);
		if (stream.isNull()) {
			LE("Cannot open hyphenation dictionary %s", UnicodeToUtf8(_filename).c_str());
			return false;
		}
        TexHyph * method = new TexHyph();
        if ( !method->load( stream ) ) {
...
#if DUMP_PATTERNS==1
                    LD("Pattern matched: %s %s on %s %s", LCSTR(lString16(p->word)), p->attr, LCSTR(lString16(s)), mask);
#endif
                    p->apply(mask);
                    found = true;
                }
...
//            lChar16 ch = hyph.wl;
//            LD("wl=%s mask=%c%c", LCSTR(lString16(&ch, 1)), hyph.mask0[0], hyph.mask0[1]);
...
#if DUMP_PATTERNS==1
                LD("Pattern: '%s' - %s", LCSTR(lString16(pattern->word)), pattern->attr );
#endif
                addPattern( pattern );
                patternCount++;
            }
...
#if DUMP_PATTERNS==1
            LD("Pattern: (%s) '%s' - %s", LCSTR(data[i]), LCSTR(lString16(pattern->word)), pattern->attr);
#endif
            addPattern( pattern );
            patternCount++;
        }
                         ...
                         lUInt16 hyphCharWidth, lUInt16 maxWidth )
{
    ...
    LV("word to hyphenate: '%s'", LCSTR(lString16(word)));
#endif
...
//            buf2 << (lChar16)'-';
    ...
    LV("Hyphenate: %s  %s", LCSTR(buf), LCSTR(buf2) );
#endif
