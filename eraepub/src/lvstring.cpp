...
const lString8 & cs8(const char * str) {
    ...
    for (;;) {
        ...
        if (p == str) {
            ...
            LV("allocating static string8 %s", str);
#endif
...
#if DEBUG_STATIC_STRING_ALLOC == 1
            LV("allocating static string16 %s", str);
#endif
...
#if DEBUG_STATIC_STRING_ALLOC == 1
            LV("allocating static string16 %s", LCSTR(str));
#endif
            const_ptrs_16[index] = str;
            size_16++;
            values_16[index] = lString16(str);
...
//    }
    ...
    //LD("Word bounds: '%s'", LCSTR(lString16(str+start, end-start)));
}
...
lString16 lString16::TrimEndQuestionChar(lString16 & str){
    ...
    //LD("TrimEndQUestionChar: %s", UnicodeToUtf8(str).c_str());
    return str;
}
