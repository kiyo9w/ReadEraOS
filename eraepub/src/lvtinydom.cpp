...
#include <../../freetype/include/freetype/internal/psaux.h>
...
//#define DEBUG_RENDER_RECT_ACCESS
  ...
  static void rr_lock( ldomNode * node )
  {
    ...
    LV("RenderRectAccessor(%d) lock", index );
...
bool ldomTextStorageChunk::setParent( int offset, lUInt32 parentIndex )
{
    ...
    LE("Offset %d is out of bounds (%d) for storage chunk %c%d, chunkCount=%d",
...
{
    if ( !_buf ) {
        LE("Modified is called for node which is not in memory");
    }
}
...
lUInt16 CrDomXml::getAttrNameIndex(const lChar16* name) {
    ...
    if (item) {
    	//LW("Found item: %s", LCSTR(lString16(name)));
        return item->id;
    }
    _attrNameTable.AddItem(_nextUnknownAttrId, lString16(name), NULL);
    //LW("Added item: %s", LCSTR(lString16(name)));
...
bool CrDom::setRenderProps(int width, int height, font_ref_t def_font, int interline_space)
{
 ...
 /*   if (_imgScalingOptions.zoom_in_inline.mode != DEF_IMAGE_SCALE_ZOOM_IN_MODE) {
    ...
    if ( _last_docflags != getDocFlags() ) {
        //LV("ldomDocument::setRenderProps() - doc flags changed");
        _last_docflags = getDocFlags();
        changed = true;
    }
    if ( calcHash(_def_style) != calcHash(s) ) {
        //LV("ldomDocument::setRenderProps() - style is changed");
        _def_style = s;
        changed = true;
    }
    if ( calcHash(_def_font) != calcHash(def_font)) {
        //LV("ldomDocument::setRenderProps() - font is changed");
        _def_font = def_font;
        changed = true;
    }
    if ( _page_height != height ) {
        //LV("ldomDocument::setRenderProps() - page height is changed");
        _page_height = height;
        changed = true;
    }
    if ( _page_width != width ) {
        //LV("ldomDocument::setRenderProps() - page width is changed");
        _page_width = width;
        changed = true;
    }
    //{
    ...
    //    LV("Style hash before set root style: %x", styleHash);
    //}
    ...
    //    lUInt32 styleHash = calcStyleHash();
    //    styleHash = styleHash * 31 + calcGlobalSettingsHash();
    //    LV("Style hash after set root style: %x", styleHash);
    //}
    return changed;
}
    ...
    */
    ...
    if (!stylesheet_file_name_.empty()) {
        ...
        if (parseStyleSheet(stylesheet_file_name_)) {
        	LV("applyDocStylesheet() using stylesheet from link/stylesheet: %s",
    ...
    if (ss.isNull()) {
        LV("applyDocStylesheet() /FictionBook/stylesheet not found");
        return;
    }
    lString16 css = ss.getText('\n');
    if (css.empty()) {
        LV("applyDocStylesheet() /FictionBook/stylesheet is empty");
        return;
    }
    if(css.pos("{")==-1)
    {
        LV("applyDocStylesheet() /FictionBook/stylesheet is invalid");
        return;
    }
    LV("applyDocStylesheet() using /FictionBook/stylesheet:\n%s", LCSTR(css));
...
bool CrDom::checkRenderContext()
{
    ...
    if (node != NULL && node->getFont().isNull()) {
        LV("checkRenderContext: Style is not set for root node");
        res = false;
    }
    lUInt32 styleHash = calcStyleHash();
...
int CrDom::render(LVRendPageList* pages,
		...
		int interline_space)
{
    LI("CrDom::render w=%d, h=%d, fontFace=%s, docFlags=%d",
    ...
    //} else {
    //    LV("reusing existing format data...");
    //}

    this->ApplyEmbeddedStyles();

    if (!checkRenderContext() || force_render) {
        //LI("CrDom::checkRenderContext FORMATTING");
        dropStyles();
        //LV("stylesheet_.push()");

        stylesheet_.push();
        applyDocStylesheet();
        //LI("initNodeStyleRecursive()");
        getRootNode()->initNodeStyleRecursive();
        //LV("stylesheet_.pop()");
        stylesheet_.pop();
        //LV("Init render method");
    ...
    if (!_rendered) {
        ...
        LV("Final block count: %d", numFinalBlocks);
        //updateStyles();
        int height = renderBlockElement( context, getRootNode(), 0, y0, width ) + y0;
        _rendered = true;
        gc();
        //LV("finalizing... fonts.length=%d", _fonts.length());
        context.Finalize();
        updateRenderContext();
        _pagesData.reset();
        ...
        LV("rendering context is not changed, no render");
        if (_pagesData.pos()) {
            _pagesData.setPos(0);
            pages->deserialize( _pagesData );
        }
        LV("%d rendered pages found", pages->length());
...
void ldomElementWriter::onBodyEnter()
{
    ...
    if (_document->isDefStyleSet()) {
        ...
        if (_element->getStyle().isNull()) {
            LE("element style init error %x %s",
...
#ifdef TRACE_AUTOBOX
        LV("Autobox children %d..%d of node <%s>  childCount=%d", firstNonEmpty,
                     lastNonEmpty, LCSTR(getNodeName()), getChildCount());
        for (int i = firstNonEmpty; i <= lastNonEmpty; i++) {
            ldomNode* node = getChildNode(i);
            if (node->isText()) {
                LV("    text: %d '%s'", node->getDataIndex(), LCSTR(node->getText()));
            } else {
                LV("    elem: %d <%s> rendMode=%d  display=%d", node->getDataIndex(),
...
void ldomNode::initNodeRendMethod()
{
    ...
    //     LE("Invalid parent->child relation for nodes %d->%d",
    //     getParentNode()->getDataIndex(), getDataIndex() ); }
    //if ( getNodeName() == "image" ) { LV("Init log for image");}
    ...
    switch (d)
    {
        case css_d_none:
            ...
            break;
        case css_d_inline:
            if ((this->getNodeId() == el_span || this->getNodeId() == el_cite) && checkHasBlockChilrden(this))
            {
            ...
            break;
        case css_d_run_in:
            //LV("switch all children elements of <%s> to inline", LCSTR(getNodeName()));
            recurseElements( resetRendMethodToInline );
...
//            p = p->getParentNode();
//        }
//        LE("style not initialized for element 0x%04x %s path %s",
...
void ldomElementWriter::onText( const lChar16 * text, int len, lUInt32 )
{
    //logfile << "{t";
    {
        ...
        if ( !_isBlock || _element->getChildCount()!=0 || !IsEmptySpace( text, len ) || (_flags&TXTFLG_PRE) || (_flags&TXTFLG_KEEP_SPACES) ) {
            ...
            //LV("ldomElementWriter::onText: Ignoring first empty space of block item");
        }
    }
    //logfile << "}";
}
...
bool ldomNode::applyNodeStylesheet()
{
	...
	//LV("ldomNode::applyNodeStylesheet()");
	if (!getCrDom()->getDocFlag(DOC_FLAG_EMBEDDED_STYLES) || getNodeId() != el_DocFragment) {
		return false;
	}
	//LV("applyNodeStylesheet DOC_FLAG_EMBEDDED_STYLES=true");
...
{
    //LV("~ldomElementWriter for element 0x%04x %s",
...
void LvDomWriter::OnStart(LVFileFormatParser * parser)
{
    ...
    //LV("LvDomWriter::OnStart()");
    if ( !_headerOnly )
        _stopTagId = 0xFFFE;
    else {
        _stopTagId = doc_->getElementNameIndex(L"description");
        //LV( "LvDomWriter() : header only, tag id=%d", _stopTagId );
    }
...
void LvDomWriter::OnTagClose(const lChar16*, const lChar16* tagname) {
    ...
    if (tagname[0] == 'l' && _currNode && !lStr_cmp(tagname, "link")) {
            ...
            )
        {
            ...
            //LV("Internal stylesheet file: %s", LCSTR(stylesheetFile));
    ...
    if ( isStyleSheetTag ) {
        ...
        if ( !_popStyleOnFinish ) {
            //LV("saving current stylesheet before applying of document stylesheet");
...
ldomXPointer CrDom::convertXPointerFromMuPdf(const lString16 & xPointerStr )
{
    ...
    //LE("ps = %d, pe = %d",pagestart,pageend);
    //LE("bs = %d, be = %d",blockstart,blockend);
    //LE("ls = %d, le = %d",linestart,lineend);
    //LE("cs = %d, ce = %d",charstart,charend);
    ...
    {
        LE("blocknode creation failed: newstr = %s",LCSTR(newstr));
        return ldomXPointer();
    }
...
ldomXPointer CrDom::createXPointer(lvPoint pt, int direction)
{
    ...
    if (!finalNode) {
        ...
        LV("createXPointer not final node");
        return ptr;
    }
    lvRect rc;
    finalNode->getAbsRect(rc);
    //LV("CrDom::createXPointer point = (%d, %d), finalNode %08X rect = (%d,%d,%d,%d)",
    ...
    for (int l = 0; l < lcount; l++) {
        ...
        for (int w = 0; w < wc; w++) {
            ...
            if (x < word->x + word->width || w == wc - 1) {
                ...
                //LV(" word found [%d]: x=%d..%d, start=%d, len=%d  %08X",
...
bool ldomXPointer::getRect(lvRect &rect, bool forlvpoint) const
{
    ...
    LD("OLD getrect Start");
    //LV("ldomXPointer::getRect()");
    #endif
    if (isNull())
    {
        #if DEBUG_GETRECT_LOGS
        LE("OLD XPOINTER IS NULL RETURN");
        #endif
        return false;
    }
    ...
    if (!p)
    {
        //LV("ldomXPointer::getRect() - p==NULL");
    }
    //printf("getRect( p=%08X type=%d )\n", (unsigned)p, (int)p->getNodeType() );
    if (!p->getCrDom())
    {
        //LV("ldomXPointer::getRect() - p->getCrDom()==NULL");
    }
    ...
    if (finalNode == NULL)
    {
        ...
        //LV("node w/o final parent: %d..%d", rc.top, rc.bottom);
    }
    ...
    if (finalNode != NULL)
    {
        ...
        #if DEBUG_GETRECT_LOGS
        //LE("OLD FINAL NODE = [%s]",LCSTR(finalNode->getXPath()));
        //LE("OLD FINAL NODE TEXT = [%s]",LCSTR(finalNode_->getText()));
        //LE("OLD NODE      = [%s]",LCSTR(node->getXPath()));
        //LE("OLD NODE TEXT = [%s]",LCSTR(node->getText()));
        #endif
        ...
        if (srcIndex == -1)
        {
            if (lastIndex < 0)
            {
                return false;
                #if DEBUG_GETRECT_LOGS
                //LE("LAST INDEX < 0 RETURN");
                #endif
            }
           #if DEBUG_GETRECT_LOGS
            //LE("OLD offset = %d",offset);
            //LE("OLD lastoffset = %d",lastOffset);
            //LE("OLD replacing offset with last offset");
           #endif
            srcIndex = lastIndex;
            srcLen = lastLen;
            offset = lastOffset;
        ...
        #if DEBUG_GETRECT_LOGS
        LE("srcIndex = %d srcLen = %d lastIndex = %d lastLen = %d lastOffset = %d", srcIndex, srcLen, lastIndex, lastLen, lastOffset);
        #endif
        for (int l = 0; l < txtform->GetLineCount(); l++)
        {
            ...
            for (int w = 0; w < (int) frmline->word_count; w++)
            {
                ...
                {
                    #if DEBUG_GETRECT_LOGS
                    //LE("word->src_text_index > srcIndex || offset <= word->t.start");
                    //LE("%d>%d || %d <= %d",word->src_text_index,srcIndex,offset,word->t.start);
                    //LE("(offset < word->t.start + word->t.len) || (offset == srcLen && offset == word->t.start + word->t.len)");
                    //LE("(%d < %d + %d) || (%d == %d && %d == %d + %d)",offset, word->t.start , word->t.len,offset , srcLen ,offset , word->t.start , word->t.len);
                    ...
                    if (word->src_text_index > srcIndex || offset <= word->t.start)
                    {
                        ...
                        #if DEBUG_GETRECT_LOGS
                        //LE("word->x = %d, rc.left = %d, frmline->x = %d",word->x,rc.left,frmline->x);
                        LE("l = %d, w = %d lastword = %d",l,w,(lastWord)?1:0);
                        LE("old Rect1 = [%d:%d][%d:%d]",rect.left,rect.right,rect.top,rect.bottom);
                        #endif
                        return true;
                    }
                    else if ((offset < word->t.start + word->t.len) || (offset == srcLen && offset == word->t.start + word->t.len))
                    {
                      // pointer inside this word
                        ...
                        #if DEBUG_GETRECT_LOGS
                        LE("l = %d, w = %d lastword = %d",l,w,(lastWord)?1:0);
                        LE("old Rect2 = [%d:%d][%d:%d]",rect.left,rect.right,rect.top,rect.bottom);
                        #endif
                        return true;
                    }
                    else if (lastWord)
                    {
                        ...
                        #if DEBUG_GETRECT_LOGS
                        LE("l = %d, w = %d lastword = %d",l,w,(lastWord)?1:0);
                        LE("old Rect3 = [%d:%d][%d:%d]",rect.left,rect.right,rect.top,rect.bottom);
                        #endif
                        return true;
                    }
                }
    ...
    else
    {
        ...
        if (offset < 0 || node->getChildCount() == 0)
        {
            #if DEBUG_GETRECT_LOGS
            //LE("offset = %d",offset);
            //LE("childcoount = %d",node->getChildCount());
...
{
    //LV( "CrDom::createXPointer(%s)", UnicodeToUtf8(xPointerStr).c_str() );
    ...
    while ( *str ) {
        //LV( "    %s", UnicodeToUtf8(lString16(str)).c_str() );
        step_type = ParseXPathStep( str, name, index );
        //LV( "        name=%s index=%d", UnicodeToUtf8(lString16(name)).c_str(), index );
        switch (step_type ) {
...
//                    LV( "node[%d]: %d %s", i, p->getNodeId(), LCSTR(p->getNodeName()));
...
//                    //LV("    Element %d is not found. foundCount=%d", id, foundCount);
...
lString16 ExtractDocAuthors(CrDom* dom, lString16 delimiter)
{
    ...
    for (int i = 0; i < 64; i++) {
        ...
        if (!pauthor) {
            //LV( "xpath not found: %s", UnicodeToUtf8(path).c_str() );
            break;
        }
        ...
        LVArray<ldomWord>& words, int maxCount, int maxHeight)
{
    ...
    LV("ldomDocument::findText() for Y %d..%d, range %d..%d",
            minY,
            maxY,
            start.toPoint().y,
            end.toPoint().y);
    if (range.getStart().toPoint().y == -1) {
        ...
        LV("ldomDocument::findText() updated range %d..%d",
    ...
    if (range.isNull()) {
        LV("No text found: Range is empty");
        return false;
    }
...
void ldomXRange::forEach2(ldomNodeCallback *callback)
{
    ...
    //LE("start  path = %s",LCSTR(start->getXPath()));
    //LE("end    path = %s",LCSTR(end->getXPath()));
    //LE("parent path = %s",LCSTR(common_parent->getXPath()));
    //LE("parent lvl  = %d",common_parent->getNodeLevel());
    ...
    while (parent->getNodeLevel() >= level)
    {
        for (int i = index; i < parent->getChildCount(); i++)
        {
            ...
            if (node->isText())
            {
                //LE("TEXT nodepath = %s",LCSTR(node->getXPath()));
                callback->processText(node, this);
            }
            else
            {
                //LE("ELEMENT nodepath = %s",LCSTR(node->getXPath()));
            ...
            if (endfound)
            {
                //LE("FOREACH FINISH nodepath = %s",LCSTR(node->getXPath()));
                return;
            }
        }
        index = parent->getNodeIndex() + 1;
        parent = parent->getParentNode();
        //LE("parentpath = %s",LCSTR(parent->getXPath()));
        if (parent == end)
        {
            //LE("PARENT EXIT");
            return;
...
void ldomNodeCallback::processText(ldomNode* node, ldomXRange * range)
{
    ...
    LE("full  text = %s",LCSTR(node->getText()));
    LE("substrtext = %s",LCSTR(text));
    */
}
...
bool ldomNodeCallback::processElement(ldomNode* node, ldomXRange * range)
{
    for (lUInt32 i = 0; i < node->getChildCount(); i++)
    {
        ...
        {
            //LE("_______ELEMENT nodepath = %s",LCSTR(child->getXPath()));
...
void ldomXRange::getRangeChars(LVArray<TextRect>& words_list,int clip_width, bool rtl_enable, bool rtl_space ) {
    ...
    public:
        ...
        virtual void onText(ldomXRange* nodeRange) {
            ...
            lString16 text = node->getText();
            //LE("node [%s]",LCSTR(node->getXPath()));
            //LE("Text[%s]",LCSTR(text));
            //LE("Textlen = %d",text.length());
...
lString16 LvDocFragmentWriter::convertHref( lString16 href )
{
...
#if 0
    LV("convertHref(%s, codeBase=%s, filePathName=%s)",
            LCSTR(href),
            LCSTR(codeBase),
            LCSTR(filePathName));
#endif
    ...
    if ( !href.split2(cs16("#"), p, id) )
        p = href;
    if ( p.empty() ) {
        //LV("codebase = %s -> href = %s", LCSTR(codeBase), LCSTR(href));
        if ( codeBasePrefix.empty() )
            return href;
        p = codeBasePrefix;
    } else {
        ...
        //LV("href %s -> %s", LCSTR(p), LCSTR(replacement));
        if ( !replacement.empty() )
            p = replacement;
        else
            return href;
    ...
    //LV("converted href=%s to %s", LCSTR(href), LCSTR(p) );
...
void LvDocFragmentWriter::setCodeBase( lString16 fileName )
{
    ...
    if ( codeBasePrefix.empty() ) {
        //LV("codeBasePrefix is empty for path %s", LCSTR(fileName));
        codeBasePrefix = pathSubstitutions.get(fileName);
    }
		...
		const lChar16* attrname, const lChar16* attrvalue)
{
    if ( insideTag ) {
        if ( !lStr_cmp(attrname, "href") || !lStr_cmp(attrname, "src" )|| !lStr_cmp(attrname, "nref" ) ) {
            ...
            //LV("name attribute = %s", LCSTR(lString16(attrvalue)));
        ...
        if ( styleDetectionState ) {
            ...
            if (styleDetectionState == 15) {
                ...
                //LV("CSS file href: %s", LCSTR(stylesheetFile));
            }
        }
    }
}
    ...
    if ( !insideTag /*&& baseTag==tagname*/ ) {
        ...
        if ( !baseTagReplacement.empty() ) {
            ...
            if (!stylesheetFile.empty()) {
                ...
                //LV("Setting StyleSheet attribute to %s for document fragment",
...
{
    if (!_tagBodyCalled) {
        LE("LvDomAutocloseWriter::OnTagOpen w/o parent's OnTagBody : %s",
...
{
    if ( !_tagBodyCalled ) {
        LE("OnTagClose w/o parent's OnTagBody : %s", LCSTR(lString16(tagname)));
        crFatalError();
    }
    //logfile << "LvDomWriter::OnTagClose() [" << nsname << ":" << tagname << "]";
    //if ( nsname && nsname[0] )
...
void LvDomAutocloseWriter::ElementCloseHandler(ldomNode* node)
{
    ...
    if (parent) {
        ...
        if (id == el_table) {
            ...
            //LV("DIV attr align = %s", LCSTR(node->getAttributeValue(attr_align)));
            //LV("DIV attr count = %d", node->getAttrCount());
            //int alignId = node->getCrDom()->getAttrNameIndex("align");
            //LV("align= %d %d", alignId, attr_align);
            //for (int i = 0; i < node->getAttrCount(); i++)
            //    LV("DIV attr %s", LCSTR(node->getAttributeName(i)));
...
lUInt32 CrDomBase::calcStyleHashFull()
{
    ...
    /*
    LV("calcStyleHash: elemCount=%d, globalHash=%08x, docFlags=%08x",
            _elemCount,
            globalHash,
            docFlags);
    */
...
bool CrDomBase::validateDocument()
{
    ...
    for (int i = 0; i < count; i++) {
        ...
        for (int j = 0; j < sz; j++) {
            ...
            if (!style) {
                if (!buf[j].isRoot()) {
                    LE("styleId=0 for node <%s> %d",
                            LCSTR(buf[j].getNodeName()),
                            buf[j].getDataIndex());
                    res = false;
                }
            } else if (_styles.get(style).isNull()) {
                LE("styleId!=0, but absent in cache for node <%s> %d",
                        LCSTR(buf[j].getNodeName()),
                        buf[j].getDataIndex());
                res = false;
            }
            if (!font) {
                if (!buf[j].isRoot()) {
                    LE("fontId=0 for node <%s>", LCSTR(buf[j].getNodeName()));
                    res = false;
                }
            } else if (_fonts.get(font).isNull()) {
                LE("fontId!=0, but absent in cache for node <%s>",
                        LCSTR(buf[j].getNodeName()));
                res = false;
            }
        }
    ...
    for (int i = 0; i < count; i++) {
        ...
        for (int j = 0; j < sz; j++) {
            ...
            if (!s.isNull()) {
                ...
                if (fntIndex <= 0) {
                    LE("font caching failed for style!");
                    res = false;
                } else {
                ...
                LE("Loaded style index %d not found in style collection", (int) style);
                setNodeFontIndex(buf[j]._handle._dataIndex, 0);
                setNodeStyleIndex(buf[j]._handle._dataIndex, 0);
                //buf[j]._data._pelem._styleIndex = 0;
                //buf[j]._data._pelem._fontIndex = 0;
...
void CrDomXml::setStylesheet(const char* css, bool replace)
{
    ...
    if (oldHash != newHash) {
        //LV("New stylesheet hash: %08x", newHash);
    }
#endif
}
...
void ldomNode::onCollectionDestroy()
{
    if ( isNull() )
        return;
    //LV("ldomNode::onCollectionDestroy(%d) type=%d", this->_handle._dataIndex, TNTYPE);
    switch ( TNTYPE ) {
    case NT_TEXT:
...
void ldomNode::destroy()
{
    if ( isNull() )
        return;
    //LV("ldomNode::destroy(%d) type=%d", this->_handle._dataIndex, TNTYPE);
    switch ( TNTYPE ) {
    case NT_TEXT:
        delete _data._text_ptr;
        break;
...
void ldomNode::setParentNode( ldomNode * parent )
{
    ...
    if (getParentNode() != NULL && parent != NULL) {
        LV("Changing parent of %d from %d to %d",
                getDataIndex(),
...
/// recursively looks for child node with certain nodename
bool ldomNode::hasChild(const char *name)
{
    if (this->isNodeName(name))
    {
        return true;
...
/// recursively looks for child node with certain nodename
ldomNode * ldomNode::getChild(const char *name)
{
    if (this->isNodeName(name))
    {
        return this;
    ...
    {
        ldomNode *child = getChildNode(i);
        ldomNode *img  = child->getChild(name);
        if(img!= NULL)
        {
            return img;
        }
...
public:
    ...
    void printOut()
    {
        for (int i = 0; i < string_.length(); i++)
        {
            LE("[l %lc l] (0x%4x) ={ %s }",string_.at(i),string_.at(i),LCSTR(pointers_.get(i)->getXPath()));
        }
    }
...
bool ldomNode::initNodeFont()
{
    ...
    if ( fntIndex==0 ) {
        ...
        if ( s.isNull() ) {
            LE("style not found for index %d", style);
...
#ifdef TRACE_AUTOBOX
    LV("moveItemsTo() invoked from %d to %d",
            getDataIndex(),
    ...
    if (refName.empty()) {
        ...
        if (!recindex.empty()) {
            ...
            if (recindex.atoi(n)) {
                ...
                //LV("get mobi image %s", LCSTR(refName));
            }
        }
//        else {
...
LVImageSourceRef ldomNode::getObjectImageSource()
{
    ...
    if (!ref.isNull()) {
        ...
        LW("ObjectImageSource cannot be opened by name:%s", LCSTR(refName));
    }
    ...
    for (int i = list; i > 0; i--) {
        ...
        if (url.startsWithNoCase(lString16("res://")) ||
            ...
            if (!fontMan->RegisterExternalFont(item->getUrl(),
                    ...
                    item->getBold(),
                    item->getItalic())) {
                LE("Failed to register external font face: %s file: %s",
                        item->getFace().c_str(),
                        LCSTR(item->getUrl()));
            }
            continue;
            ...
            if (!fontMan->RegisterDocumentFont(getDocIndex(),
                    ...
                    item->getBold(),
                    item->getItalic())) {
                LE("Failed to register document font face: %s file: %s",
                        item->getFace().c_str(),
                        LCSTR(item->getUrl()));
...
LVStreamRef CrDom::getObjectImageStream( lString16 refName )
{
    ...
    if ( refName.startsWith(lString16(BLOB_NAME_PREFIX)) ) {
        ...
        if ( !getDocParentContainer().isNull() ) {
            ...
            */
            if ( ref.isNull() ) {
                LE("Cannot open stream by name %s", LCSTR(name));
...
int ldomNode::renderFinalBlock(  LFormattedTextRef & frmtext, RenderRectAccessor * fmt, int width )
{
    ...
    if ( cache.get( this, f ) ) {
        ...
        if ( rm != erm_final && rm != erm_list_item && rm != erm_table_caption )
            return 0;
        //RenderRectAccessor fmt( this );
        //LV("Found existing formatted object for node #%08X", (lUInt32)this);
    ...
    frmtext = f;
    //LV("Created new formatted object for node #%08X", (lUInt32)this);
    return h;
}
...
//#define TINYNODECOLLECTION_DUMPSTATISTICS
    ...
    LV("CrDomBase::dumpStatistics: totalNodes: %d (%d kB)\n"
...
{
    //LE("ldomWord getrect start");
    lvRect result;
    ldomXRange range(this->getStartXPointer(),this->getEndXPointer());
    range.getRect(result);
    //LE("ldomWord getrect end");
    return result;
}

bool isHeader(ldomNode * node)
{
    return (node->isNodeName("h1") ||
            node->isNodeName("h2") ||
            node->isNodeName("h3") ||
            node->isNodeName("h4") ||
            node->isNodeName("h5") ||
...
void CleanupDocxHeaders(ldomNode * parent)
{
    for (int i = 0; i < parent->getChildCount(); i++)
    {
        ldomNode * child = parent->getChildNode(i);
        ...
        if (isHeader(child))
        {
            lString16 newtext = child->getText();

            bool found = false;
            //LE("child   [%s]", LCSTR(child->getXPath()));
            ldomNode * sibling = child->getNextSibling();
            while(!sibling->isNull())
            {
                if(isHeader(sibling))
                {
                    //LE("sibling [%s]", LCSTR(sibling->getXPath()));
                    newtext += sibling->getText();
                    ldomNode * n_sibling = sibling->getNextSibling();
                    parent->removeChild(sibling->getNodeIndex());
                    sibling = n_sibling;
                    found = true;
                }
                else
            ...
            if(found)
            {
                int idx = child->getNodeIndex();
                int nsid = child->getNodeNsId();
                int id = child->getNodeId();
                parent->removeChild(idx);
                ldomNode *newnode = parent->insertChildElement(idx, 0, id);
                newnode->insertChildText(newtext);
            }
        }
        else
        {
            CleanupDocxHeaders(child);
        }
    }
}
...
void PrettyPrintDocx(ldomNode * node)
{
    ...
    for (int i = 0; i < node->getChildCount(); i++)
    {
        ...
        if (child->isNodeName("p"))
        {
            ...
            {
                //LE("before str = %s",LCSTR(bound.string_));
    ...
    for (int i = 0; i < node->getChildCount(); i++)
    {
        ...
        if(!deepSearch)
        {
            if (child->isNodeName("h1") || child->isNodeName("h2") || child->isNodeName("h3"))
            {
                ...
                //LE("added toc [%s], path = [%s]",LCSTR(child->getText()),LCSTR(child->getXPath()));
            }
...
void RecurseDocxBookmarkTOC(ldomNode * node,LvTocItem * toc)
{
    if(node->isNodeName("body") && node->hasAttribute(attr_name) && node->getAttributeValue(attr_name)=="notes_hidden")
    {
        return;
    ...
    lString16 recordedText = lString16::empty_str;
    ...
    for (int i = 0; i < node->getChildCount(); i++)
    {
        ...
        if(child->isElement())
        {
            if (child->isNodeName("bookmarkstart") || child->isNodeName("bookmark-start"))
            {
                recording = true;
                continue;
            }
            if (child->isNodeName("bookmarkend") || child->isNodeName("bookmark-end"))
            {
                recording = false;
                if (recordedText.empty() || recordedText == "-")
                {
                    continue;
                }
                if (recordedText.length() > TOC_ITEM_LENGTH_MAX)
                {
                    recordedText = recordedText.substr(0, TOC_ITEM_LENGTH_MAX);
                    recordedText = recordedText + lString16("...");
                }
                recordedText.trim();

                auto *item = new LvTocItem(ldomXPointer(child, 0), child->getPath(), recordedText);
                toc->addItem(item, 1);
                //LE("added DOCX toc item [%s], path = [%s]", LCSTR(recordedText), LCSTR(child->getXPath()));
                continue;
            }
            if (recording)
            {
            ...
            {
                RecurseDocxBookmarkTOC(child,toc);
            }
        }
    }

    //if bookmarkstart was called but bookmark end was not (still recording but no children left)- save all we have read on this level
    if(recording)
    {
        LE("Still recording!");
        recording = false;
        if (!recordedText.empty())
        {
            if (recordedText.length() > TOC_ITEM_LENGTH_MAX)
            {
                recordedText = recordedText.substr(0, TOC_ITEM_LENGTH_MAX);
            ...
            recordedText.trim();
            LvTocItem *item = new LvTocItem(ldomXPointer(node, 0), node->getPath(), recordedText);
            toc->addItem(item, 1);
            //LE("added DOCX toc item [%s], path = [%s]", LCSTR(recordedText), LCSTR(node->getXPath()));
        }
    }
}

void GetDocxBookmarkTOC(CrDom * crDom, LvTocItem * toc)
{
    ldomNode * root = crDom->getRootNode();
    for (int i = 0; i < root->getChildCount(); i++)
    {
...
void FixOdtSpaces(ldomNode * node)
{
    for (int i = 0; i < node->getChildCount(); i++)
    {
        ...
        if(child->isText())
        {
            ...
            if (s->isNull())
            {
                //LE("get sibling from upper node");
