...
public:
    ...
    void addFileItem( const char * filename, LONGUINT64 len )
    {
        ...
        //LV("CHM file item: %s [%d]", filename, (int)len);
        Add(item);
    }
    ...
    bool decodeBlock( const lUInt8 * ptr, lUInt32 blockOffset, int size ) {
        ...
        while ( data + 8 < maxdata ) {
            ...
            if ( data < maxdata ) { //urlOffset > offset ) {
                ...
                //LV("urlstr[offs=%x, url=%s]", item->offset, item->url.c_str());
                _table.add( item );
            }
        }
        return true;
...
public:
    static CHMUrlStr * open( LVContainerRef container ) {
        ...
        LI("CHM URLSTR: %d entries read", res->_table.length());
        return res;
    }
...
class CHMUrlTable {
    ...
    bool decodeBlock( const lUInt8 * data, lUInt32 offset, int size ) {
        for ( int i=0; i<URLTBL_BLOCK_RECORD_COUNT && size>0; i++ ) {
            ...
            //LV("urltbl[offs=%x, id=%x, ti=%x, urloffs=%x]",
...
public:
    ...
    static CHMUrlTable * open( LVContainerRef container ) {
        ...
        LI("CHM URLTBL: %d entries read", res->_table.length());
        return res;
    }
...
class CHMSystem {
    ...
    bool decodeEntry() {
        ...
        //LV("CHM binary item code=%d, length=%d, bytesLeft=%d", code, length, _reader.bytesLeft());
        if ( err )
            return false;
        LVArray<lUInt8> bytes;
        switch( code ) {
		...
		_language = langToLanguage( _lcid );
                if ( enc_name!=NULL ) {
                    ...
                    LI("CHM LCID: %08x, charset=%s", _lcid, LCSTR(_enc_name));
                } else {
                    LI("CHM LCID: %08x -- cannot find charset encoding table", _lcid);
                }
        ...
        case 16:
            ...
            LI("CHM default font: %s", _defaultFont.c_str());
            if ( _enc_table==NULL ) {
                for ( int i=_defaultFont.length()-1; i>0; i-- ) {
                    if ( _defaultFont[i]==',' ) {
                        ...
                        if ( cpname!=NULL && table!=NULL ) {
                            LI("CHM charset detected from default font: %s",
    ...
    bool read() {
        ...
        if ( err ) {
            LE("CHM decoding error: %d blocks decoded, stream bytes left=%d",
        ...
        if ( node!=NULL ) {
            for ( int i=0; i<node->getChildCount(); i++ ) {
                ...
                if (child && child->isElement() && child->getNodeName() == "param" && child->getAttributeValue(L"name") == "Font") {
                    ...
                    LD("LVParseCHMHTMLStream: encoding detected: %d", encoding);
                }
            }
        }
...
public:
    ...
    {
        //LV("CHM toc level %d: '%s' : %s", level, LCSTR(name), LCSTR(url) );
        if (url.startsWith(".."))
...
//                    LV("unsorted %d : %s", j, LCSTR(htms[j]));
//                }
//            }
            htms.sort(filename_comparator);
//            {
//                for (int j=0; j<htms.length(); j++) {
//                    LV("sorted %d : %s", j, LCSTR(htms[j]));
//                }
//            }
            urlList.addAll(htms);
        }
    ...
    int appendFragments(bool needs_coverpage)
    {
        ...
        for ( int i=0; i<cnt; i++ ) {
            ...
            if ( parser.CheckFormat() && parser.Parse() ) {
                ...
                LE("Document type is not HTML for fragment %s", LCSTR(fname));
            }
            appendedFragments++;
        }
...
bool ImportCHMDocument(LVStreamRef stream, CrDom* doc, bool cfg_firstpage_thumb_)
{
    ...
    writer.OnStop();
    LD("CHM: %d documents merged", fragmentCount);
