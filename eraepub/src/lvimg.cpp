...
cr_jpeg_error (j_common_ptr cinfo)
{
    ...
    //LE("cr_jpeg_error : returning control to setjmp point %08x", &myerr->setjmp_buffer);
...
public:
    ...
    {
    	//LV("creating LVJpegImageSource");

        // testing setjmp

//        jmp_buf buf;
//        if (setjmp(buf)) {
//            LV("longjmp is working ok");
//            return;
//        }
//        longjmp(buf, -1);
    }
    ...
    virtual bool   Decode( LVImageDecoderCallback * callback )
    {
    	//LV("LVJpegImageSource::decode called");
...
//        LD("Opening node image id=%s", LCSTR(id));
//    }
...
public:
    ...
    virtual void OnStartDecode( LVImageSource * )
    {
        //LV( "LVUnpackedImgSource::OnStartDecode" );
    }
    ...
    {
        //LV( "LVUnpackedImgSource::OnEndDecode" );
    }
    virtual ldomNode * GetSourceNode() { return NULL; }
    virtual LVStream * GetSourceStream() { return NULL; }
    virtual void   Compact() { }
...
LVImageSourceRef LVCreateUnpackedImageSource( LVImageSourceRef srcImage, int maxSize, bool gray )
{
    ...
    if ( sz>maxSize )
        return srcImage;
    LV("Unpacking image %dx%d (%d)", dx, dy, sz);
