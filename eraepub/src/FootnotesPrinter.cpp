...
void FootnotesPrinter::recurseNodesToPrint(ldomNode *node)
{
    ...
    //LE("node to recurse = %s",LCSTR(node->getXPath()));
...
ldomNode * FootnotesPrinter::FindTextInParents(ldomNode *node)
{
    ...
    //LE("node path = %s",LCSTR(node->getXPath()));
...
bool FootnotesPrinter::PrintLinksList(LVArray<LinkStruct> LinksList)
{
    ...
    for (int i = 0; i < LinksList.length(); i++)
    {
        ...
        if (node == NULL)
        {
            LE("Failed to get node from href = %s, skipping", LCSTR(href));
            continue;
        }
        if (node->isText())
        {
            LE("Node is Text, skipping");
            continue;
        }
        ...
        else
        {
            ...
            for (int i = index + 1; i < parent->getChildCount(); i++)
            {
                ...
                {
                    //LD("CHILD IS BREAK = %s",LCSTR(child->getXPath()));
                    break;
                }
                //LE("child path = %s",LCSTR(child->getXPath()));
                if(child->isText())
                {
                    lString16 text = child->getText();
                    int txtlen = text.length();
                ...
                {
                    //text is not a number
                    //LE("on child tag [%d] [%s]",child->getText().length(), LCSTR(child->getNodeName()));
                    if (this->textcounter_ < NOTES_HIDDEN_MAX_LEN)
                    {
