...
static const rtf_control_word * findControlWord( const char * name )
{
    ...
    for ( ;; ) {
        ...
        //    LE("findControlWord invalid index: %d of %d", c, rtf_words_count);
        //    return NULL;
        //}
...
void LVRtfParser::CommitText()
{
    ...
    lString16 s = txtbuf;
    LV("Text(%s)", UnicodeToUtf8(s).c_str());
#endif
...
#ifdef LOG_RTF_PARSING
        LV("CW: %s\\%s %d", asterisk?"\\*":"", control, param==PARAM_VALUE_NONE ? 0 : param);
#endif
        if ( asterisk ) {
            // ignore text after unknown keyword
            m_stack.set( new LVRtfNullDestination(*this) );
#ifdef LOG_RTF_PARSING
            LV("Ignoring unknown destination %s !!!", control );
#endif
        }
    }
}
