...
void CssStyle::parseShorthandMargin(const lString16& in)
{
    ...
    switch (coll.length())
    {
        ...
        default:
            ...
            margin_bottom_ = coll.at(2);
            margin_left_   = coll.at(3);
            LE("malformed margin css tag for [%s]",LCSTR(this->name_));
            break;
    }
}
...
void CssStyle::parseShorthandPadding(const lString16& in)
{
    ...
    switch (coll.length())
    {
        ...
        default:
            ...
            padding_bottom_ = coll.at(2);
            padding_left_   = coll.at(3);
            LE("malformed padding css tag for [%s]",LCSTR(this->name_));
            break;
    }
}
...
CssStyle::CssStyle(lString16 in, lString16 codebase)
{
    ...
    //if(name_.startsWith("#")) //ignoring "#" selectors, cutting "#rule" off
    //{
    //    int spacePos = name_.pos(" ");
    //    if(spacePos>=0)
    //    {
    //        spacePos++;
    //        name_ = name_.substr(spacePos,name_.length() - spacePos);
    //    }
    //}
    if(rest.empty())
    {
    ...
    //fix for stupid error mirroring text-alignment in rtl doc css styles
    if(direction_ == "rtl")
    {
        if(text_align_ == ta_right)
        {
            text_align_ = ta_left;
        }
    }
    ...
    //LE("curr style_string_fonts_margins = %s", LCSTR(style_string_fonts_margins));
};
...
lString16 CssStyle::getAttrval(lString16 in, lString16 attrname)
{
    ...
    if(attr_start == -1)
    {
        //LE("getAttrval [%s] return empty str",LCSTR(attrname));
        return result;
    }
    ...
    //LE("getAttrval [%s] return [%s]",LCSTR(attrname),LCSTR(result.trimDoubleSpaces(false, false, false)));
...
lString16 CssStyle::formatCSSstring(bool fonts, bool margins)
{
    ...
    if(str.empty())
    {
        //LE(" str.empty() class [%s], sourceline = [%s]",LCSTR(name_),LCSTR(source_line_));
...
CssStyle CssStyle::OverwriteClass(CssStyle add)
{
    ...
    //LE("merged style_string_fonts_margins is[%s]",LCSTR(style_string_fonts_margins));
...
{
    //LE("check classname = [%s]",LCSTR(name));
    if(name.empty())
    {
        return false;
    }
    for (int i = 0; i < name.length(); i++)
    {
        ...
        {
            //LE("Found illegal character in CSS class name: [%s] -> [%lc]",LCSTR(name),ch);
            return false;
        }
    }
    ...
    for (int i = 0; i < name.length(); i++)
    {
        ...
        if (last == '.' || last == ' ' || last == ',')
        {
            if ((ch >= 45 && ch <= 57) || ch == '-')
            {
                //LE("Illegal character combination in css class name: [%s] -> [%lc][%lc]",LCSTR(name),last,ch);
                return false;
            }
        }
        if(ch == '[')
...
lString16Collection EpubStylesManager::splitSelectorToClasses(lString16 selector)
{
    ...
    //LE("selector in = %s",LCSTR(selector));
...
void EpubStylesManager::addCssRTLClass(CssStyle css)
{
    if (css.isRTL())
    {
        ...
        for (int i = 0; i < names.length(); i++)
        {
            ...
            //LE("rtl class in map = %s",LCSTR(name));
        }
    }
...
lString16 EpubStylesManager::as_string()
{
    switch (gEmbeddedStylesLVL)
    {
        ...
        default: LE("embedded styles level = 0!");
            return lString16::empty_str;
    }
}
