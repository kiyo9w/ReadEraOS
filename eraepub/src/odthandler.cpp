    ...
    if (styles)
    {
        ...
        for (int i = 1; i < 50000; i++)
        {
            ...
            for (int i = 0; i < item->getChildCount(); i++)
            {
                ...
                {
                    return true;
                    //LE("autospacing = true! (val = %s)", LCSTR(value));
                    break;
                }
            }
        }
...
bool ImportOdtDocument(LVStreamRef stream, CrDom *m_doc, bool firstpage_thumb)
{
    ...
    if (arc.isNull())
    {
        LE("This ODT is corrupted: not a ZIP archive");
        return false; // not a ZIP archive
    }
    // check if there is document.xml
    ...
    if (rootfilePath.empty())
    {
        LE("No main document file found! This is either not a ODT file, or the file is corrupted!");
        return false;
    }
    //lString16 footnotesFilePath = DocxGetFootnotesFilePath(arc);
    EncryptedDataContainer *decryptor = new EncryptedDataContainer(arc);
    ...
    if (content_stream.isNull())
    {
        LE("ImportOdtDocument : failed to open rootfile %s",LCSTR(rootfilePath));
        return false;
    }
...
*/
    ...
    //LE("odtstyles setfields = %d",odtStyles.setfields);
    //LE("odtstyles 1 = %s",LCSTR(odtStyles.h1id_));
    //LE("odtstyles 2 = %s",LCSTR(odtStyles.h2id_));
    //LE("odtstyles 3 = %s",LCSTR(odtStyles.h3id_));
    //LE("odtstyles 4 = %s",LCSTR(odtStyles.h4id_));
    //LE("odtstyles 5 = %s",LCSTR(odtStyles.h5id_));
    //LE("odtstyles 6 = %s",LCSTR(odtStyles.h6id_));
    LVArray<LinkStruct> LinksList;
    LinksMap LinksMap;
    ...
    if (!stream2.isNull())
    {
        ...
        {
            LE("Unable to parse odt file at [%s]", LCSTR(rootfilePath));
        }
    }
