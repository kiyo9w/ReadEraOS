    ...
    int LookupElem( ldomNode * el, int state ) {
        ...
        for (int i=0; i<el->getChildCount(); i++) {
            ...
            if ( item ) {
                ...
                //LV("LookupElem[%d] (%s, %d) %d", i, LCSTR(item->getNodeName()), state, (int)item->getRendMethod() );
                switch ( rendMethod ) {
    ...
    void PlaceCells() {
        ...
        for (i=0; i<rows.length(); i++) {
            for (j=0; j<rows[i]->cells.length(); j++) {
                ...
                if(txtlen == 0 && (cell->elem->hasChild("img") || cell->elem->hasChild("image")))
                {
                    txtlen = 1;
                }
                if(cell->colspan > 1)
        ...
        if (colcheck > 0)
        {
            ...
            for (int i = 0; i < cols.length(); i++)
            {
                ...
                for (int j = 0; j < rows.length(); j++)
                {
                    ...
                    lUInt32 cellwidth = 0;
                    if(cell->elem->hasChild("img") || cell->elem->hasChild("image") )
                    {
                        ldomNode * child = cell->elem->getChild("img");
                        if(child == NULL)
                        {
                            child = cell->elem->getChild("image");
                        }
                        LVImageSourceRef img = child->getObjectImageSource();
                        if(img.isNull())
                        {
                            continue;
                        }
                        cellwidth = img->GetWidth();
                    }
                    else
                    {
                        ...
                        cellwidth = font->getTextWidth((text + "   ").c_str(), text.length() + 3);
                    }
...
//        if ( ntype ) {
//            LV("Node %s is Object ?  %d", LCSTR(enode->getNodeName()), ntype->is_object );
//        } else {
//            LV("Node %s (%d) has no css_elem_def_props_t", LCSTR(enode->getNodeName()), enode->getNodeId() );
//        }
...
//        if ( isFootNoteBody )
//            LV("renderBlockElement() : Footnote body detected! %s", LCSTR(ldomXPointer(enode,0).toString()) );
        //if (!fmt)
        //    crFatalError();
...
//            x = x;
        ...
        LFormattedTextRef txform;
        {
            //LV("renderBlockElement - creating render accessor");
            ...
            case erm_table_cell:
                {
                    ...
                    //    LV("rendering final node: %s %d %s", LCSTR(enode->getNodeName()), enode->getDataIndex(), LCSTR(ldomXPointer(enode,0).toString()) );
                    ...
                    //    LV("last rendered node: %s %d", LCSTR(enode->getNodeName()), enode->getDataIndex());
    #ifdef DEBUG_DUMP_ENABLED
                    logfile << "\n";
    #endif
            ...
            case erm_invisible:
                ...
                return 0;
            default:
                LE("Unsupported render method %d", m);
                crFatalError(); // error
                break;
            }
        }
        if ( flgSplit ) {
            ...
            if ( context.getPageList() != NULL ) {
                ...
                //if (before!=css_pb_auto) {
                //    LV("page break before node %s class=%s text=%s", LCSTR(enode->getNodeName()), LCSTR(enode->getAttributeValue(L"class")), LCSTR(enode->getText(' ', 120) ));
                //}
...
void ApplyInlineStyle(ldomNode * enode, css_style_rec_t* pstyle)
{
    ...
    if (gEmbeddedStylesLVL > 0 && enode->hasAttribute(LXML_NS_ANY, attr_style)) {
        ...
        if (!nodeStyle.empty())
        {
            ...
            //LE("CssStyle filtered : [%s] -> [%s]",LCSTR(nodeStyle),LCSTR(Style.style_string_));
            nodeStyle = Style.getStyleStringFonts();
            LVCssDeclaration decl;
            lString8 s8 = UnicodeToUtf8(nodeStyle);
            const char* s = s8.c_str();
            if (decl.parse(s)) {
                //LE("style applied to node [%s] = [%s]",LCSTR(enode->getXPath()),LCSTR(nodeStyle));
...
void setNodeStyleRend(ldomNode* enode, css_style_ref_t parent_style, LVFontRef parent_font)
{
    ...
    if (parent_style.isNull()) {
        LE("setNodeStyleRend: parent style is null");
    }
#endif
