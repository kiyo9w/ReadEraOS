...
void LVCssDeclaration::apply(const ldomNode* node, css_style_rec_t* style)
{
    if (!_data) {
#ifdef OREDEBUG
        //LI("CSS: no declaration for %s", LCSTR(GetNodeDesc(node)));
#endif
        return;
    }
#ifdef DEBUG_CSS
    //LV("CSS: apply declaration [%s]", LCSTR(GetNodeDesc(node)));
#endif
    int* p = _data;
    for (;;) {
        switch (*p++) {
...
bool LVCssSelectorRule::check(const ldomNode*& node)
{
    ...
    case cssrt_parent: {
        ...
        //LV("        cssrt_parent: %d %d", node->getNodeId(), _id);
        if (node->isNull()) {
            LV("        cssrt_parent_class node->isNull()");
            return false;
        }
    ...
    case cssrt_parent_class: {
        ...
        val.lowercase();
        //LV("        cssrt_parent_class: [%s]==[%s]", LCSTR(val), LCSTR(_value));
        return val == _value;
    }
        break;
    case cssrt_ancessor: {
    ...
    case cssrt_predecessor: {
        ...
        if (elem) {
            node = elem;
            //LV("+ selector: found pred element");
            return true;
        }
        return false;
    }
    ...
    case cssrt_attreq: {
        ...
        //if ( res )
        //    return true;
        //LV("attreq: %s %s", LCSTR(val), LCSTR(_value) );
        return res;
    }
        break;
    case cssrt_attrhas: {
    ...
    case cssrt_universal:
        ...
        return true;
    default:
        LE(" LVCssSelectorRule::check() rule type is undefined.");
        return false;
    }
    return true;
}
...
bool LVCssSelector::check(const ldomNode* node) const
{
...
#ifdef DEBUG_CSS
        LV("    selector miss [%s] (reason: id)", name_.c_str());
#endif
        return false;
    }
    if (!_rules) {
#ifdef DEBUG_CSS
        LD("    selector match [%s] (reason: no rules)", name_.c_str());
#endif
        return true;
    }
    if (node->isNull()) {
#ifdef OREDEBUG
        LI("    selector miss [%s] (reason: null node)", name_.c_str());
#endif
        return false;
    }
    if (node->isRoot()) {
#ifdef OREDEBUG
        //LV("    selector miss [%s] (reason: root node)", name_.c_str());
#endif
        return false;
    }
    ...
    do {
        if (!rule->check(n)) {
#ifdef DEBUG_CSS
            LV("    selector miss [%s] rule check", name_.c_str());
#endif
            return false;
        }
        rule = rule->getNext();
...
#ifdef DEBUG_CSS
    LD("    selector match [%s]", name_.c_str());
#endif
    return true;
}
...
void LVStyleSheet::applyCss(const ldomNode* node, css_style_rec_t* style)
{
...
#ifdef OREDEBUG
        LI("LVStyleSheet::applyCss[%s]: selectors null", LCSTR(GetNodeDesc(node)));
#endif
        return;
    }
    bool applied = false;
    ...
    if( node->hasAttribute(attr_class))
    {
        ...
        classname = L"." + classname;
        lString8 classname8 = UnicodeToUtf8(classname);
        //LE("looking for %s",classname8.c_str());
        if(selectorMap_.find(classname8.getHash()) != selectorMap_.end())
        {
            ...
            //LE("%s found: %x",classname8.c_str(),selector_class);
        }
        else
        {
            //LE("couldnt find class %s in map",LCSTR(classname));
...
#ifdef DEBUG_CSS
    if (id == 0) {
        LI("LVStyleSheet::applyCss[%s]: node id==0", LCSTR(GetNodeDesc(node)));
    } else if (!selector_id) {
        LV("LVStyleSheet::applyCss[%s]: !selector_id", LCSTR(GetNodeDesc(node)));
    } else {
        LV("LVStyleSheet::applyCss[%s]", LCSTR(GetNodeDesc(node)));
    }
#endif
    for (;;) {
        LVCssSelector *sel = NULL;
...
#ifdef DEBUG_CSS
                //LV("property name: %s", lString8(res, str - res).c_str());
#endif
                skip_spaces_and_comments(str);
                res = str;
...
#ifdef OREDEBUG
    LD("parse_property_name cssd_unknown");
#endif
    return cssd_unknown;
}
...
bool LVCssSelector::parse(const char*& str, CrDomXml* dom)
{
...
#ifdef OREDEBUG
        LE("LVCssSelector::parse: empty string");
#endif
        return false;
    }
    if(!skip_spaces_and_comments(str))
    {
        LE("LVCssSelector::parse: empty string 2");
        return false;
    }
    char selector[512];
    int i;
    ...
    for (;;) {
...
#ifdef OREDEBUG
                LI("LVCssSelector::parse fail: !parse_ident");
#endif
                return false;
            }
...
#ifdef OREDEBUG
            LI("LVCssSelector::parse fail: unknown selector type");
#endif
            return false;
        }
        if (*str == ',' || *str == '{') {
        ...
        while (*str == '[' || *str == '.' || *str == '#') {
            ...
            if (!rule) {
#ifdef OREDEBUG
                LI("LVCssSelector::parse fail: parse_attr");
#endif
                return false;
            }
        ...
        if (!attr_rule) {
#ifdef OREDEBUG
            LI("LVCssSelector::parse fail: !attr_rule");
#endif
            return false;
        } else if (*str == ',' || *str == '{') {
            return true;
...
bool LVStyleSheet::parse(const char* str)
{
    ...
    for (; *str;) {
        ...
        for (; *str;) {
            ...
            if (!decl->parse(str)) {
#ifdef OREDEBUG
                LI("LVCssDeclaration::parse fail");
#endif
                err = true;
                err_count++;
            } else {
...
//#ifdef OREDEBUG
            ...
            //        str++;
            //    }
            //    LI("LVStyleSheet::parse error: %s", css_error.c_str());
            //}
//#else
            while (*str && *str != '}') {
                str++;
...
void LVStyleSheet::GenerateClassMap()
{
    ...
    while (sel != NULL)
    {
        //LE("sel add to map =  %s, %x",sel->name_.c_str(),sel);
