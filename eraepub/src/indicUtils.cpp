...
bool char_isIndic(int ch)
{
    return ( CharIsDvng(ch)     ||
             CharIsBangla(ch)   ||
             CharIsGujarati(ch) ||
             CharIsKannada(ch)  ||
             CharIsMalay(ch)    ||
...
bool checkIndic(std::wstring wstring)
{
    return std::any_of(wstring.begin(), wstring.end(), [](wchar_t ch){ return char_isIndic(ch); } );
}
...
void lString16::checkCJK()
{
    ...
    for (int i = 0; i < this->length(); i++)
    {
        int ch = this->at(i);
        #if CJK_PATCH
        if(UnicodeHelper::isCJKIdeograph(ch))
        {
            gDocumentCJK = 1;
            return;
        }
...
bool lString16::checkCJ_bool()
{
    //return std::all_of(wstring.begin(), wstring.end(), [](wchar_t ch){ return isChineseJapaneseIdeograph(ch); } );

    //ANY OF CHARS IS CJ
    for (int i = 0; i < this->length(); i++)
    {
        if(UnicodeHelper::isChineseJapaneseIdeograph(this->at(i)))
        {
            return true;
        }
    }
...
bool lString16::checkK_bool()
{
    //return std::all_of(wstring.begin(), wstring.end(), [](wchar_t ch){ return isKoreanIdeograph(ch); } );

    //ANY OF CHARS IS CJ
    for (int i = 0; i < this->length(); i++)
    {
        if(UnicodeHelper::isKoreanIdeograph(this->at(i)))
        {
            return true;
        }
    }
...
{
    lString16 result = input;
    while (UnicodeHelper::isPunct(result.firstChar()) || UnicodeHelper::isSpace(result.firstChar()))
    {
        result = result.substr(1);
    }
    while (UnicodeHelper::isPunct(result.lastChar()) || UnicodeHelper::isSpace(result.lastChar()))
    {
...
{
    lString16 result = input;
    while (UnicodeHelper::isSpace(result.firstChar()))
    {
        result = result.substr(1);
    }
    while (UnicodeHelper::isSpace(result.lastChar()))
    {
        result = result.substr(0, result.length() - 1);
    }
    return result;
