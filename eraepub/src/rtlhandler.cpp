...
public:
    ...
    bool hasPunct(){
        ...
        return UnicodeHelper::isPunct(ch);
    }
    ...
    bool checkLineRTL()
    {
        ...
        //LE("line length = %d",line.length());

        for (int i = 0; i < list_.length(); i++)
        {
            lChar16 ch = list_.get(i).getText().firstChar();
            if(UnicodeHelper::isRTL(ch))
            {
                return true;
            }
        }
...
int getSpaceWidth(LVArray<TextRectGroup> words)
{
    ...
    //LE("orig width = %d",orig_line_width);
    for (int i = start; i < words_len; i++)
    {
        ...
        if(curr.getText().firstChar() == ' ' )
        {
            ...
            //LE("space counter = %d",space_counter);
            //LE("space rect width = %d",curr.getRectWidth());
            //LE("space font width = %d",curr.getFontWidth(font));
        ...
        //LE("curr = [%s]",LCSTR(curr.getText()));
        //LE("real left = %d , real right = %d",left,right);
        //LE("width = %d, rectwidth %d",width,curr.list_.get(0).getRect().width());
        //LE("l/r1 = [%d:%d], width = %d, offset = %d gaps = %d",left,right,width, offset, gaps);

        left = (rectheight > fontheight)? right : left;
        int gap = left - offset;
        //LE("hitbox word left = %d, text = [%s]",left+100,LCSTR(curr.getText()));
        //LE("gap = left - offset = %d = %d - %d",left-offset,left,offset);
        offset += width + gap;
        gaps += gap;
        //LE("l/r2 = [%d:%d], width = %d, gap = %d , offset = %d gaps = %d",left,right,width,gap, offset, gaps);
    }

    if (space_counter > 0)
    {
        int spacewidth = (gaps + gapfix) / space_counter;
        //LI("gaps = %d,gapfix = %d, space_counter = %d , spacewidth = %d",gaps, gapfix,space_counter, spacewidth);
        return spacewidth;
    }
...
LVArray<TextRectGroup> reverseWordsOrder(LVArray<TextRectGroup> words, int spacewidth, int clip_width)
{
    ...
    //LE("first text = [%s], left = %d",LCSTR(first_text),first_rect.left);
    ...
    //LE("first left = %d",startx);
    //LE("rfirst left = %d",first_rect.left);
    //LE("rfirst right = %d",first_rect.right);
    ...
    for (int w = words.length()-1; w >= 0; w--)
    {
        ...
        if(currword.is_rtl_ || (currword.hasPunct() && prev_state ))
        {
            ...
            {
                /*
                LV("nonrtlbuff start 1");
                for (int i = 0; i < nonRTLBuffer.length(); i++)
                {
                    LE("nonrtlbuffer %d , [%s]",i, LCSTR(nonRTLBuffer.get(i).getText()));
                }
                 */
                ...
                for (int b = nonRTLBuffer.length()-1; b >=0  ; b--)
                {
                    ...
                    //LE("word from buff = [%s], is rtl = %d, startx_nonrtlbuff = %d", LCSTR(buff_word.getText()), (int) buff_word.is_rtl_, startx_nonrtlbuff);
            ...
            //LE("rtl word after buff = [%s], is rtl = %d, startx = %d, startx nonrtlbuff = %d", LCSTR(currword.getText()), (int) currword.is_rtl_, startx, startx_nonrtlbuff);
            if(startx<startx_nonrtlbuff)
            {
                //LV("moving startx right by %d pixels",startx_nonrtlbuff-startx);
        ...
        {
            // LE("word to buff = [%s], is rtl = %d, startx = %d", LCSTR(currword.getText()), (int) currword.is_rtl_, startx);
    ...
    if(nonRTLBuffer.length()>0)
    {
        ...
        {
            TextRectGroup buff_word = nonRTLBuffer.get(b);
            //LE("nonrtl word in end from buff = [%s], is rtl = %d, startx = %d", LCSTR(buff_word.getText()), (int) buff_word.is_rtl_, startx_nonrtlbuff);
...
/*TextRectGroup LigatureCheck(TextRectGroup word, LVFont * font)
{
    ...
    if(!isRTL(word.getText().firstChar()))
    {
        return word;
    }
...
LVArray<TextRect> reverseLine(TextRectGroup group, int clip_width)
{
    ...
    //LE("WORDS BREAKUP START");
    int start = 0;
    bool last_space = false;
    bool last_punct = false;

    lString16 last_text = line.get(0).getText();
    bool last_state = UnicodeHelper::isRTL(last_text.firstChar());
    ...
    word.is_rtl_ = last_state;
    words.add(word);
    //LE("added word = [%s]  (%s)",LCSTR(word.getText()),(word.is_rtl_)?"RTL":"NOT RTL");
    //LE("WORDS BREAKUP END");
...
/*    for (int i = 0; i < words.length(); i++)
    ...
    */
    ...
    //    TextRectGroup curr = words.get(i);
    //    int left =  curr.list_.get(0).getRect().left;
    //    LE("IN hitbox word left = %d, text = [%s]",left,LCSTR(curr.getText()));
    //}
    ...
    for (int i = 0; i < words.length(); i++)
    {
        for (int j = 0; j < words.get(i).list_.length(); j++)
        {
            ...
            //if(isRTL(curr.getText().firstChar()))
            //{
...
TextRect getZeroTxRect(TextRectGroup *line)
{
    ...
    //LE("zero l/r = %d / %d, height = %d fontheight = %d",zerorect.left,zerorect.right,zerorect.height(),font->getHeight());
    return result;
}
...
LVArray<TextRect> RTL_mix(LVArray<TextRect> in_list, int clip_width, bool rtl_space)
{
    ...
    for (int l = 0; l < lines.length(); l++)
    {
        if(lines[l].is_rtl_ && lines[l].checkLineRTL())
        {
            ...
            //LV("reverse line text = [%s]",LCSTR(lines[l].getText()));
...
std::vector<lString16> lString16::ArabicSplitToWords()
{
    ...
    bool last_state = UnicodeHelper::isRTL(last_text);
...
bool lString16::CheckRTL()
{
    ...
    {
        if (UnicodeHelper::isRTL(this->at(i)))
        {
            return true;
        }
    }
