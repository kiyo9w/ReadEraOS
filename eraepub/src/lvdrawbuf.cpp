...
//            LV("frame[%d, %d] src=%d dst=%d %d -> %d", frame1, frame2, src_len, dst_len, i, map[i]);
//            if (map[i] >= src_len) {
//                LE("Wrong coords");
//            }
        }
...
{
    if ( dx==_dx && dy==_dy ) {
    	//LV("LVColorDrawBuf::Resize : no resize, not changed");
    	return;
    }
    if ( !_ownData ) {
    	//LV("LVColorDrawBuf::Resize : no resize, own data");
        return;
    }
    //LV("LVColorDrawBuf::Resize : resizing %d x %d to %d x %d", _dx, _dy, dx, dy);
    // delete old bitmap
...
void LVGrayDrawBuf::DrawRescaled(LVDrawBuf * src, int x, int y, int dx, int dy, int options)
{
    ...
    //LV("LVGrayDrawBuf::DrawRescaled bpp=%d %dx%d srcbpp=%d (%d,%d) (%d,%d)", _bpp, GetWidth(), GetHeight(), src->GetBitsPerPixel(), x, y, dx, dy);
	CHECK_GUARD_BYTE;
            ...
            if (linearInterpolation) {
...
#if 1
            	{
            		if (_ownData && _data[_rowsize * _dy] != GUARD_BYTE) {
            			LE("lin interpolation, corrupted buffer, yy=%d of %d", yy, dy);
                ...
                {
            		if (_ownData && _data[_rowsize * _dy] != GUARD_BYTE) {
            			LE("area avg, corrupted buffer, yy=%d of %d", yy, dy);
            			crFatalError(-5, "corrupted bitmap buffer");
            		}
            	}
#endif
