...
struct MobiPreamble : public PalmDocPreamble
{
    ...
    bool read( LVStreamRef stream, lUInt16 & extraDataFlags ) {
        ...
        /*if ( compression!=1 && compression!=2 )
        {
            LE("Compression is Huffman/cdic compression! Abort!");
            return false;
        }*/
...
//            if (extraDataFlags) {
//                LV("extraDataFlags=%04x", (int)extraDataFlags);
//            }
        }
        return true;
    }
...
public:
    ...
    virtual LVStreamRef OpenStream( const lChar16 * fname, lvopen_mode_t mode ) {
        ...
        for ( int i=0; i<_list.length(); i++ ) {
            //LV("OpenStream(%s) : %s", LCSTR(lString16(fname)), LCSTR(lString16(_list[i]->GetName())) );
    ...
    void removeExtraData(int index, LVArray<lUInt8> & buf) {
        ...
        for (int flag = 0x8000; flag; flag >>= 1) {
            ...
            if (flag == 1) {
                ...
                if (n>0) {
                    //LV("block %d: removing %d bytes of multibyte character", index, n);
                ...
                if (n > 0 && buf.length() >= n) {
                    //LV("block %d: removing %d bytes of extra data type %d", index, n, flag);
...
//                if (flag == 1 && n > 1) {
//                    LV("block %d: removing %d bytes of multibyte character", index, n - 1);
        ...
        if (_format == EREADER) {
                        ...
                        if (recLen > 8) {
                            ...
                            if (recLen == 12 && recType == 201) {
                                ...
                                if (recType == 100) {
                                    ...
                                    LV("MOBI author: %s", author.c_str());
                                } else if (recType == 105) {
                                    lString8 s((const char*) buf2.get(), recLen - 8);
                                    LV("MOBI subject: %s", s.c_str());
                                }
                            }
                            //================================
            ...
            if (container) {
                for (int index = preamble.firstImageIndex; index < _records.length(); index++) {
                    ...
                    //LD("Image record %d [%02x %02x %02x %02x %02x]", index, buf[0], buf[1], buf[2], buf[3], buf[4]);
                    const char* fmt = NULL;
                    ...
                    if (fmt) {
                        ...
                        //LD("Adding image %s [%d] %s", LCSTR(name), _records[index].size, fmt);
                        container->addItem(new LVPDBRegionContainerItem(stream,
                                this,
                                name,
...
//        }
        ...
        if (_textSize == (lUInt32) -1)
            ...
            LW("PDB: Unpacked text size is %d but expected %d", unpoffset, _textSize);
            _textSize = unpoffset;
