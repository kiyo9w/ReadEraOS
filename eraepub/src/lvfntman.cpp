...
void LVFontManager::SetGammaIndex( int index ) {
    ...
    if ( gammaIndex!=index ) {
        //LV("FontManager gamma index changed from %d to %d", gammaIndex, index);
        gammaIndex = index;
...
//    gammaIndex = GAMMA_LEVELS/2;
    ...
    if ( gammaIndex!=oldGammaIndex ) {
        //LV("FontManager gamma index changed from %d to %d", oldGammaIndex, gammaIndex);
        clearGlyphCache();
    }
}
...
bool isRotatable(lChar16 ch)
{
    ...
    for (lChar16 c : forced_rotatable)
    {
        if(ch == c)
            return true;
    }
...
public:
    ...
    bool loadFromBuffer(LVByteArrayRef buf, int index, int size, css_font_family_t fontFamily, bool monochrome, bool italicize )
    {
        ...
        //LV("Loaded font %s [%d]: faceName=%s, ", _fileName.c_str(), index, _faceName.c_str());
    ...
    */
    virtual bool getGlyphInfo(lUInt16 code, glyph_info_t * glyph, lChar16 def_char=0)
    {
        ...
        //LE("code       = %c = %d = %x",code,code,code);
        //LE("glyphindex = %lc = %d = %x",glyph_index,glyph_index,glyph_index);
        if (glyph_index != 0)
        {
        ...
        //LE("Current font face :%s",this->getTypeFace().c_str());
        //LE("Current fallback face : %s", curface.c_str());
        if  (SYSTEM_FALLBACK_FONTS_ENABLE)
        {
            fontMan->InitFallbackFonts();
        }
        ...
        while (fontMan->AllowFallbackCycle())
        {
            ...
            if (glyph_index != 0)
            {
                LV("Fallback face: %s", nextface.c_str());
                return fallback->getGlyphInfoItem(glyph_index, glyph);
            }
        }
            ...
            lUInt32 *palette, bool addHyphen, lUInt32 flags, int letter_spacing, bool reverse, int maxWidth)
    {
        ...
        for (int i=0; i<=len; i++) {
            ...
            if ( i<len ) {
                ...
                if(reverse)
                {
                    if ( ch == ']' ) { ch = '['; LE("char replaced"); } else if ( ch == '[' ){ ch = ']';LE("char replaced");}
                    if ( ch == '}' ) { ch = '{'; LE("char replaced"); } else if ( ch == '{' ){ ch = '}';LE("char replaced");}
                    if ( ch == ')' ) { ch = '('; LE("char replaced"); } else if ( ch == '(' ){ ch = ')';LE("char replaced");}
                }
                isHyphen = (ch==UNICODE_SOFT_HYPHEN_CODE) && (i<len-1);
...
public:
    ...
    virtual void SetFallbackFace(int index)
    {
        ...
        LE("Setting new fallbackfontface!");
    ...
    virtual bool SetFallbackFontFace( lString8 face ) {
        if ( face!=_fallbackFontFace ) {
          ...
          //  LV("Looking for fallback font %s", face.c_str());
            LVFontCacheItem * item = _cache.findFallback( face, -1 );
            if (!item)
            {
             LE("Face %s not found", face.c_str());
                face.clear();
            }
    ...
    virtual void InitFallbackFonts()
    {
        ...
        LV("Fallback fonts initialisation...");
    ...
    virtual void InitFallbackFontDefault()
    {
        ...
        LV("Default fallback font initialisation...");
        AddFallbackFontFaceIntoArray(FALLBACK_FACE_DEFAULT);

        if (_fallbackFontFaceArrayLength == 0)
        {
            LE("Default fallback face not found!!");
            return;
        }

        lString8 fallbackface = GetFallbackFontFaceFromArray(0);
        SetFallbackFontFace(fallbackface);
        LV("Default fallback face set : %s",fallbackface.c_str());
    ...
    virtual void SetHintingMode(hinting_mode_t mode) {
        if (_hintingMode == mode)
            return;
        //LV("Hinting mode is changed: %d", (int)mode);
        _hintingMode = mode;
        gc();
        clearGlyphCache();
        LVPtrVector< LVFontCacheItem > * fonts = _cache.getInstances();
    ...
    : _library(NULL), _globalCache(GLYPH_CACHE_SIZE)
    {
        ...
        if ( error ) {
            // error
        	LE("Error while initializing freetype library");
        }
    #if (DEBUG_FONT_MAN==1)
    ...
    bool setalias(lString8 alias,lString8 facename,int id,bool italic, bool bold)
    {
        ...
        if (!item->getDef()->getName().empty()) {
            ...
            for ( ;; index++ ) {
                ...
                if ( error ) {
                    if (index == 0) {
                        LW("FT_New_Face returned error %d", error);
                    }
                    break;
                }
                ...
                if ( _cache.findDuplicate( &def2 ) ) {
                    LV("font definition is duplicate");
                    return false;
                }
    ...
    virtual LVFontRef GetFont(int size, int weight, bool italic, css_font_family_t family, lString8 typeface, int documentId)
    {
        ...
        if (!item->getFont().isNull())
        {
            ...
            if ( deltaWeight >= 200 ) {
                // embolden
                LV("font: apply Embolding to increase weight from %d to %d", newDef.getWeight(), newDef.getWeight() + 200 );
...
//            }
    ...
    bool checkCharSet(FT_Face face)
    {
        ...
        for (int i = 0; i < _requiredChars.length(); i++) {
            ...
            FT_UInt ch_glyph_index = FT_Get_Char_Index(face, ch);
            if (ch_glyph_index == 0) {
                //LV("Required char (%04x) not found in font %s", ch, face->family_name);
                // No required char
               // return false;
            }
        }
    ...
    LVArray<LanguageWrap> initLangCheckArray()
    {
        ...
        _langCheckArray.add(LanguageWrap(L"ru",    L"CYR",L"АаБбВвГгДдЕеЖжЗзИиЙйКкЛлМмНнОоПпРрСсТтУуФфХхЦцЧчШшЩщЪъЫыЬьЭэЮюЯя"));
        ...
        _langGroupCheckArray.add(LanguageWrap(L"CYR",L"CYR",L"абвгдежзиклмнопрстуфхцчшщьыъэюяАБВГДЕЖЗИКЛМНОПРСТУФХЦЧШЩЬЫЪЭЮЯ"));
    ...
    */
    ...
    virtual bool RegisterDocumentFont(int documentId, LVContainerRef container, lString16 name, lString8 faceName, bool bold, bool italic) {
        ...
        //LV("RegisterDocumentFont(documentId=%d, path=%s)", documentId, name8.c_str());
        ...
        for ( ;; index++ ) {
            ...
            if ( error ) {
                if (index == 0) {
                    LW("FT_New_Memory_Face returned error %d", error);
                }
                break;
            }
//            bool scal = FT_IS_SCALABLE( face );
...
//            }
    ...
    virtual lString8Collection RegisterFont(lString8 name, bool langcheck, bool fallback_mode)
    {
        ...
        //LV("font file name : %s", fname.c_str());
#if (DEBUG_FONT_MAN == 1)
        if (_log) {
        ...
        for ( ;; index++ )
        {
            ...
            if ( error ) {
                if (index == 0) {
                    LW("FT_New_Face returned error %d for %s",
                            error, name.c_str());
                }
                break;
            }
			...
			if ( !fallback_mode && _cache.findDuplicate( &def ) ) {
                LI("Font definition is duplicate %s", def.getName().c_str());
...
{
    if (_typeface != face) {
        //LV("'%s'' != '%s'", face.c_str(), _typeface.c_str());
        return 0;
    }
...
LVFontCacheItem * LVFontCache::findFallback( lString8 face, int size )
{
    ...
    {
        //LE("Typeface %d =%s",i,_registered_list[i]->_def.getTypeFace().c_str());
...
LVFontCacheItem * LVFontCache::find( const LVFontDef * fntdef )
{
    ...
    for (int nindex=0; nindex==0 || nindex<list.length(); nindex++)
    {
        ...
        for (i=0; i<_registered_list.length(); i++)
        {
         //LE("regtypeface #%d :%s",i,_registered_list[i]->_def.getTypeFace().c_str());
...
void LVFontCache::gc()
{
    ...
    for (int i=_instance_list.length()-1; i>=0; i--) {
        if (_instance_list[i]->_fnt.getRefCount() <= 1) {
			LV("dropping font instance %s[%d] by gc()",
