...
//        LV("c font = %08x  txt = %08x", (lUInt32)font, (lUInt32)text);
...
//                        m_flags[pos] |= LCHAR_DEPRECATED_WRAP_AFTER;
    ...
    {
        //LV("resizeImage (%dx%d) max %dx%d %s", width, height, maxw, maxh, isInline ? "inline" : "block");
...
//                    return; // no zoom
    ...
    {
        //LV("Resize image (%dx%d) max %dx%d %s  *%d", width, height, maxw, maxh, arbitraryImageScaling ? "arbitrary" : "integer", maxScaleMult);
        if ( maxScaleMult<1 )
            maxScaleMult = 1;
...
#if CJK_PATCH
                  || UnicodeHelper::isCJKIdeograph(m_flags[i])
#endif
					)) {
...
//                        LV("last char is UNICODE_NO_BREAK_SPACE");
...
//                            LV("Double nbsp text[-1]=%04x", m_text[wstart-1]);
//                        else
    ...
    void processParagraph( int start, int end )
    {
        ...
        for (;pos<m_length;) {
            ...
            for ( i=pos; i<m_length; i++ ) {
                ...
                #if CJK_PATCH
                if(UnicodeHelper::isCJKIdeograph(m_text[i])           &&
                    (i + 1) < m_length                 &&
                    !(m_flags[i + 1] & LCHAR_IS_SPACE) &&
                    !UnicodeHelper::isPunct(m_text[i+1])         &&
                    !UnicodeHelper::isCJKPunctuation(m_text[i+1]) )
                {
                    lastNormalWrap = i;
                }
                #endif //CJK_PATCH
...
#if TRACE_LINE_SPLITTING==1
                if ( len>0 ) {
                    LV("wordBounds(%s) unusedSpace=%d wordWidth=%d", LCSTR(lString16(m_text+start, len)), unusedSpace, m_widths[end]-m_widths[start]);
            ...
            if ( lastMandatoryWrap>=0 )
                ...
                if ( wrapPos <= upSkipPos ) {
                    ...
                    //LV("guard old wrapPos at %d", wrapPos);
                    wrapPos = upSkipPos+1;
                    //LV("guard new wrapPos at %d", wrapPos);
                    upSkipPos = -1;
                }
            }
            ...
            if(gDocumentCJK)
            {
                ...
                int downSkipCount = 0;
                int upSkipCount = 0;
                if (endp > 1 && UnicodeHelper::isCJKLeftPunctuation(m_text[endp]))
                {
                    // Next char will be fine at the start of next line.
                    //LV("skip skip punctuation %lc, at index %d", m_text[endp], endp);
                }
                else if (endp > 1 && endp < m_length - 1 && UnicodeHelper::isCJKLeftPunctuation(m_text[endp - 1]))
                {
                    ...
                    endp--;
                    wrapPos--;
                    //LV("up skip left punctuation %lc, at index %d", m_text[endp], endp);
                }
                else if (endp > 1 && UnicodeHelper::isCJKPunctuation(m_text[endp]))
                {
                    ...
                    {
                        if (!UnicodeHelper::isCJKPunctuation(m_text[epos]))
                            break;
                        //LV("down skip punctuation %lc, at index %d", m_text[endp], endp);
                    }
                    for (int epos = endp; epos >= start; epos--, upSkipCount++)
                    {
                        if (!UnicodeHelper::isCJKPunctuation(m_text[epos]))
                            break;
                        //LV("up skip punctuation %lc, at index %d", m_text[endp], endp);
                    }
                    if (downSkipCount <= upSkipCount && downSkipCount <= 2 && false)
                    {
                        ...
                        //LV("finally down skip punctuations %d", downSkipCount);
                    }
                    else if (upSkipCount <= 2)
                    {
                        ...
                        endp -= upSkipCount;
                        wrapPos -= upSkipCount;
                        //LV("finally up skip punctuations %d", upSkipCount);
                    }
                }
            }
#endif // CJK_PATCH
...
int getSpaceWidth(LVArray<WordItem> words)
{
    ...
    for (int i = start; i < words_len; i++)
    {
        ...
        int left = curr.x_;
        //LE("hitbox word left = %d, text = [%s]",left+100,LCSTR(curr.getText()));
        int width = curr.width_;
        int gap = left - offset;
        offset += width + gap;
        gaps += gap;
        //LE("l/r = [%d:X], width = %d, gap = %d , offset = %d gaps = %d, spaces = %d",left,width,gap, offset, gaps, space_counter);
    }

    if (space_counter > 0)
    {
        int spacewidth = gaps / space_counter;
        //LE("gaps = %d, space_counter = %d , spacewidth = %d",gaps,space_counter, spacewidth);
        //LE("space_counter = %d , spacewidth = %d",space_counter, spacewidth);
        return spacewidth;
    }
    //LE("gaps = %d, space_counter = %d , spacewidth = -1",gaps,space_counter);
    //LE("space_counter = %d , spacewidth = -1",space_counter);
    return -1;
}
    ...
    for (i=0; i<m_pbuffer->frmlinecount; i++)
    {
        ...
        if (line_y + frmline->height>=clip.top)
        {
            ...
            for (j=0; j<frmline->word_count; j++)
            {
                ...
                //LV("word = [%s]",LCSTR(lString16( srcline->t.text + word->t.start,word->t.len)));
                if (word->flags & LTEXT_WORD_IS_OBJECT)
                {
                    srcline = &m_pbuffer->srctext[word->src_text_index];
                ...
                else
                {
                    ...
                    */
                    ...
                    //LE("x = %d, word->width = %d, ++ = %d",x + frmline->x + word->x,word->width,x + frmline->x + word->x+word->width);
                    if (frmline->rtl || (RTL_DISPLAY_ENABLE && gDocumentRTL == 1 && lString16(str, word->t.len).CheckRTL()))
                    {
                        printrtl = true;
                        //LE("word start = %d, len = %d",word->t.start,word->t.len);
                        //LE("str = [%s]",LCSTR(lString16(str,word->t.len)));
                        int start = 0;
                        bool last_space = false;
                        bool last_punct = false;
                        bool last_state = UnicodeHelper::isRTL(str[0]);
                        int width = 0;
                        int fullwidth = 0;
                        ...
                        for (int c = 0; c < word->t.len; c++)
                        {
                            lChar16 ch = str[c];
                            bool is_space = ch == ' ';
                            bool is_punct = UnicodeHelper::isPunct(ch);
                            bool curr_state;
                            ...
                            {
                                curr_state = UnicodeHelper::isRTL(ch);
                            }
                            ...
                            if (curr_state != last_state || break_char)
                            {
                                ...
                                if(len>0)
                                {
                                    WordItem wordItem(
                                            ...
                                            last_state,
                                            width);
                                    //LE("fmt word = [%s], x = %d , orig x = %d, width = %d",LCSTR(lString16(str+start,len)),offset,x + frmline->x + word->x,width);
                        ...
                        //LE("word->width = %d, fullwidth = %d, width = %d, last_space = %d",word->width,fullwidth,width,(int)last_space);
                        lString16(str+start,len);
                        width = (last_space)? word->width - fullwidth : width ;
                        //LE("width after = %d",width);
                        WordItem wordItem(
                                offset,
                                line_y + (frmline->baseline - font->getBaseline()) + word->y,
                                str + start,
            ...
            if (printrtl)
            {
                ...
                //    LE("fmt left = %d, text = [%s]",curr.x_,LCSTR(curr.getText()));
                //}
