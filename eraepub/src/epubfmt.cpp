...
bool ImportEpubDocument(LVStreamRef file_stream, CrDom *m_doc, bool firstpage_thumb)
{
	LVContainerRef arc = LVOpenArchive(file_stream);
	if (arc.isNull())
    {
...
//        }
        ...
        }
        {
            ...
            for (int i = 1; i < nodes_to_parse; i++)
            {
                ...
                if (epubItem->mediaType == "application/xhtml+xml" ||
                	epubItem->mediaType == "text/html")
                {
                    bool allowed = true;
                    for (int j = 0; j < NotesItems.length(); j++)
                    {
                        if (epubItem->href == NotesItems.get(j)->href)
						{
							allowed = false;
							break;
						}
	...
	if (!ncxHref.empty())
	{
		...
		lString16 codeBase_ncx = LVExtractPath(ncxHref);
		if (codeBase_ncx.length() > 0 && codeBase_ncx.lastChar() != '/')
		{
            codeBase_ncx.append(1, L'/');
		}
		appender.setCodeBase(codeBase_ncx);
		if (!stream.isNull())
		{
			...
			if (ncxdoc != NULL)
			{
				...
				if (navMap != NULL)
				{
					...
					LE("read [%d] items from ncx [%s]",m_doc->getToc()->countAllDescendants(),LCSTR(ncxHref));
				}
				delete ncxdoc;
			}
		}
	...
	//fallback for toc
	if (m_doc->getToc()->countAllDescendants() <= 1)
	{
        ...
        if (m_doc->getToc()->getChildCount() == 0)
		{
			...
			if (!navHref.empty())
			{
				...
				lString16 codeBase_nav = LVExtractPath(ncxHref);
				if (codeBase_nav.length() > 0 && codeBase_nav.lastChar() != '/')
				{
                    codeBase_nav.append(1, L'/');
				}
				appender.setCodeBase(codeBase_nav);
		...
		lString16 *res_series, int *res_series_number, lString16 *res_genre, lString16 *res_annotation, int *fontcount)
{
	...
	*res_annotation = metaSanitize(dom->textFromXPath(lString16("package/metadata/description")).trim());

	*fontcount = fontcount_inner;
	*res_genre = metaSanitize(genre);
    *res_title = metaSanitize(title);
    *res_authors = metaSanitize(authors);
    *res_lang = metaSanitize(lang);
    *res_series = metaSanitize(series);
