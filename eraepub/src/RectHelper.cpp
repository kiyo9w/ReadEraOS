...
void RectHelper::Init(ldomNode *Node)
{
    ...
    InitNode(Node_);
    #if DEBUG_GETRECT_LOGS
    //LE("NEW FINALNODE = [%s]",LCSTR(finalNode_->getXPath()));
    //LE("Node_ Text = %s",LCSTR(Node_->getText()));
    #endif
}
...
void RectHelper::InitFinalNode(ldomNode *finalNode)
{
    ...
    #if DEBUG_GETRECT_LOGS
    //LE("txtform_->GetLineCount() = %d", txtform_->GetLineCount());
    //LE("finalnode = [%s]", LCSTR(finalNode_->getXPath()));
    //LE("finalnode childcount = %d", finalNode_->getChildCount());
    #endif
}
...
void RectHelper::InitNode(ldomNode *Node)
{
    ...
    {
        #if DEBUG_GETRECT_LOGS
        LE("GetSrcCount: Final node contains no text nodes!");
        #endif
    }
...
bool RectHelper::ifnull(ldomXPointerEx xpointer, lvRect &rect)
{
    ...
    //LE("NEW NODE = [%s]",LCSTR(node->getXPath()));
    //LE("Node Text = %s",LCSTR(node->getText()));
...
{
   #if DEBUG_GETRECT_LOGS
    LE("start = %d, linecount = %d", start, txtform_->GetLineCount());
   #endif
    int l = 0;
    int w = 0;
    bool found = false;
...
int RectHelper::FindNodeIndex(ldomNode *node, int start)
{
    ...
    #if DEBUG_GETRECT_LOGS
    LE("NOT FOUND NODE INDEX RETRYING");
    #endif
    ...
    {
        #if DEBUG_GETRECT_LOGS
        LE("start = %d, count = %d", start, count);
        #endif
...
int RectHelper::FindLastIndex(ldomNode *node)
{
    ...
    for (int i = 0; i < count; i++)
    {
        ...
        {
            //LE("FindLastIndex = %d",i);
            return i;
        }
    }
    #if DEBUG_GETRECT_LOGS
    LE("FindLastIndex Not found = %d", count - 1);
    #endif
...
lvRect RectHelper::getRect(ldomWord word)
{
    ...
    #if DEBUG_GETRECT_LOGS
    LE("Nodetext = [%s] , LETTER = [%s]", LCSTR(word.getNode()->getText()), LCSTR(word.getText()));
    #endif
    ...
    //if(rc1 != rc1old)
    //{
    //    LW("RC1 New rect != old rect! ^ [%d:%d][%d:%d] != [%d:%d][%d:%d]",rc1.left,rc1.right,rc1.top,rc1.bottom,rc1old.left,rc1old.right,rc1old.top,rc1old.bottom);
    //}
    //if (rc2 != rc2old)
    //{
    //    LW("RC2 New rect != old rect! ^ [%d:%d][%d:%d] != [%d:%d][%d:%d]",rc2.left,rc2.right,rc2.top,rc2.bottom,rc2old.left,rc2old.right,rc2old.top,rc2old.bottom);
    //}
...
bool RectHelper::processRect(ldomXPointerEx xpointer, lvRect &rect)
{
    ...
    if (finalNode_ == NULL)
    {
        ...
        #if DEBUG_GETRECT_LOGS
        LE("Rect ifnull = [%d:%d][%d:%d]", rect.left, rect.right, rect.top, rect.bottom);
        #endif
        return res;
    }
    ...
    //LE("absrect = [%d:%d][%d:%d]",rc.left,rc.right,rc.top,rc.bottom);
    ...
    ldomNode *node = xpointer.getNode();
    #if DEBUG_GETRECT_LOGS
    LE("NEW FINAL NODE = [%s]", LCSTR(finalNode_->getXPath()));
    // LE("FINAL NODE TEXT = [%s]",LCSTR(finalNode_->getText()));
    // LE("NODE      = [%s]",LCSTR(Node_->getXPath()));
    // LE("NODE TEXT = [%s]",LCSTR(Node_->getText()));
    #endif
    int offset = xpointer.getOffset();
    //LE("NEW offset = %d",offset);
    //LE("NEW lastoffset = %d",lastOffset_);
    if (NodeIndex_ < 0)
    {
        #if DEBUG_GETRECT_LOGS
        LE("NEW replacing offset with last offset");
        #endif
        offset = lastOffset_;
    }
    #if DEBUG_GETRECT_LOGS
    LE("srcIndex = %d srcLen = %d lastIndex = %d lastLen = %d lastOffset = %d nodeIndex =%d ", srcIndex_, srcLen_, lastIndex_, lastLen_, lastOffset_, NodeIndex_);
    #endif
    ...
    for (int l = start; l < count; l++)
    {
        ...
        for (int w = 0; w < (int) frmline->word_count; w++)
        {
            ...
            {
                #if DEBUG_GETRECT_LOGS
                LE("l = %d, w = %d lastword = %d line_index = %d", l, w, (lastWord) ? 1 : 0, LineIndex_);
                //LE("word->src_text_index > srcIndex || offset <= word->t.start");
                //LE("%d>%d || %d <= %d",word->src_text_index,srcIndex_,offset,word->t.start);
                //LE("(offset < word->t.start + word->t.len) || (offset == srcLen && offset == word->t.start + word->t.len)");
                //LE("(%d < %d + %d) || (%d == %d && %d == %d + %d)",offset, word->t.start , word->t.len,offset , srcLen_ ,offset , word->t.start , word->t.len);
                #endif
                ...
                if (word->src_text_index > srcIndex_ || offset <= word->t.start)
                {
                    ...
                    LE("Rect1 = [%d:%d][%d:%d]", rect.left, rect.right, rect.top, rect.bottom);
                    //LE("LINEINDEX END = %d",LineIndex_);
                    #endif
                    return true;
                }
                else if ((offset < word->t.start + word->t.len) || (offset == srcLen_ && offset == word->t.start + word->t.len))
                {
                    ...
                    LineIndex_ = l;
                    #if DEBUG_GETRECT_LOGS
                    LE("Rect2 = [%d:%d][%d:%d]", rect.left, rect.right, rect.top, rect.bottom);
                    //LE("LINEINDEX END = %d",LineIndex_);
                    #endif
                ...
                else if (lastWord)
                {
                    ...
                    LineIndex_ = l;
                    #if DEBUG_GETRECT_LOGS
                    LE("Rect3 = [%d:%d][%d:%d]", rect.left, rect.right, rect.top, rect.bottom);
                    //LE("LINEINDEX END = %d",LineIndex_);
                    #endif
                    return true;
                }
