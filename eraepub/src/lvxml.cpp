...
{
    //LV("LVTextFileBase::Stop() is called!");
    m_stopped = true;
}
...
bool LVTextFileBase::AutodetectEncoding( bool utfOnly )
{
    ...
    if ( sz < 16 )
    {
        LE("LVTextFileBase::AutodetectEncoding: sz<16! Trying to continue...");
        //return false;
    }
    ...
    {
        LE("LVTextFileBase::AutodetectEncoding failed to read");
        delete[] buf;
    ...
    m_stream->SetPos( oldpos );
    if ( res) {
        //LD("Code page decoding results: encoding=%s, lang=%s", enc_name, lang_name);
...
bool LVFileParserBase::Seek( lvpos_t pos, int bytesToPrefetch )
{
    ...
    if ( m_stream->SetPos( m_buf_fpos ) != m_buf_fpos ) {
        LE("cannot set stream position to %d", (int)m_buf_pos );
        return false;
    }
    lvsize_t bytesRead = 0;
    if ( m_stream->Read( m_buf, bytesToRead, &bytesRead ) != LVERR_OK ) {
        LE("error while reading %d bytes from stream", (int)bytesToRead);
        return false;
    }
    return true;
}
...
int LVTextFileBase::ReadTextBytes( lvpos_t pos, int bytesToRead, lChar16 * buf, int buf_size, int flags)
{
    if ( !Seek( pos, bytesToRead ) ) {
        LE("LVTextFileBase::ReadTextBytes seek error! cannot set pos to %d to read %d bytes", (int)pos, (int)bytesToRead);
        return 0;
    }
    int chcount = 0;
...
//        LV("fpos=%06x+%06x, sz=%04x, data: %02x %02x %02x %02x %02x %02x %02x %02x .. %02x %02x %02x %02x %02x %02x %02x %02x",
...
void LVTextFileBase::SetCharset( const lChar16 * name )
{
    ...
    if ( m_encoding_name == "utf-8" ) {
        ...
        m_enc_type = ce_8bit_cp;
        //LV("charset: %s", LCSTR(lString16(name)));
...
public:
            ...
            : flags(0), lpos(0), rpos(0), align(la_unknown)
    {
        ...
        //LD("    line read: %s", UnicodeToUtf8(text).c_str());
        ...
        if (!text.empty())
        {
            if(file->Eof())
            {
                ...
                //LD("    line fixed: %s", UnicodeToUtf8(text).c_str());
            }
...
#define MAX_PRE_STATS 1000
        ...
        for ( i=0; i<length(); i++ ) {
            ...
            //LD("   LINE: %d .. %d", line->lpos, line->rpos);
            if ( line->lpos == line->rpos ) {
                empty_lines++;
            } else {
                ...
                for (int j=line->lpos; j<line->rpos-1; j++ ) {
                    ...
                    if ( ch=='\\' ) {
                        ...
                        case 'Q':
                            ...
                            break;
                        case 'n':
                            // ignore \n tags due to frequent common usage
                            break;
                        default:
                            pmlNotTagCount++;
                        }
        ...
        LD("TxtSmartFormat() min_left=%d, max_right=%d, ident=%d, empty=%d, flags=%d",
...
//                            break;
    ...
    bool DoTextImport(LvXMLParserCallback* callback) {
        if (smart_format_flags_ & tftPML) {
            ...
            LV("tftParaEmptyLineDelim");
            return DoParaPerEmptyLinesImport(callback);
        } else {
            LV("DoParaPerLineImport");
...
lString16 LVTextFileBase::ReadLine(int maxLineSize, lUInt32& flags) {
    ...
    for (;;)
    {
        ...
        //temp.append(1, ch);
        //res.append(1, ch);
        //LV("%s %08x %d", UnicodeToUtf8(temp).c_str(), ch, ch);
        if (ch != '\r' && ch != '\n')
        {
            if ( ch<=32 )
            {
                switch (ch)
                {
                    ...
                    case 0x15:
                        ...
                        break;
                    default:  //case 0: here
                        LV("Found illegal character. Replacing with u/FFFD.");
                        res.append(1, L'\ufffd');
...
bool LvXmlParser::CheckFormat() {
    ...
    {
        LE("LvXmlParser::CheckFormat() : Failed to check format. Chars decoded = %d",charsDecoded);
    }
    delete[] chbuf;
    Reset();
    return res;
        ...
        case ps_bof:
            {
                ...
                //LV("LvXmlParser::Parse() ps_bof ret");
            }
            break;
        case ps_lt:  //look for tags
            {
            	//LV("LvXmlParser::Parse() ps_lt");
                if (!SkipSpaces())
                    break;
                close_flag = false;
                q_flag = false;
...
//                        LE("aside old = %s, new = %s",LCSTR(aside_old_id),LCSTR(aside_new_id));
                    ...
                    else
                    {
                        ...
                        if(!buffer.empty())
                        {
                            ...
                            && ((flag1 && flag2) || in_sup || in_a_sup || a_is_fb2_note))
                            {
                                ...
                                //LE("LIST added [%s] to [%s]",LCSTR(link_id),LCSTR(link_href));
                ...
                m_state = ps_attr;
                //LV("LvXmlParser::Parse() ps_lt ret");
            }
            break;
        case ps_attr: //read tags
            {
                //LV("LvXmlParser::Parse() ps_attr");
                if (!SkipSpaces())
                    break;
                ...
                if(in_aside)
                {
                    if(attrname == "id")
                    {
                        //LE("attrval = %d",("#" + callback_->convertId(attrvalue)).getHash());
                ...
                //LV("LvXmlParser::Parse() ps_attr ret");
            }
                break;
        case ps_text:
            {
            	//LV("LvXmlParser::Parse() ps_text");
//                if ( dumpActive ) {
//                    lString16 s;
...
//                }
	        ...
	        if(need_coverpage_)
			{
                //LV("LvXmlParser: text fragments read : %d", fragments_counter);
			...
			if(callback_->fb2_cover_done())
            {
	            ...
	            m_state = ps_lt;
                //LV("LvXmlParser::Parse() ps_text ret");
            }
            break;
        default:
            {
...
void LvXmlParser::initDocxTagsFilter(){
    ...
    tags.clear();
    tags_init_ = true;
    LV("DOCX tag filtering array initialized");
    return;
}
...
bool LvXmlParser::ParseDocx(DocxItems docxItems, DocxLinks docxLinks, DocxStyles docxStyles)
{
    ...
    bool first_blip_in_p = false;
    bool separate_img = false;

    bool in_td = false;
    bool in_tcpr = false;
    bool flag_colspan = false;
    bool in_imagedata = false;
    ...
    std::map<lString16,int> headermap;
    headermap[docxStyles.h1id_] = 1;
    headermap[docxStyles.h2id_] = 2;
    headermap[docxStyles.h3id_] = 3;
    headermap[docxStyles.h4id_] = 4;
    headermap[docxStyles.h5id_] = 5;
    headermap[docxStyles.h6id_] = 6;
    ...
    for (; !eof_ && !error && !firstpage_thumb_num_reached ;)
    {
        ...
        switch (m_state)
        {
            ...
            case ps_lt:  //look for tags
            {
                ...
                tagns.lowercase();
                tagname.lowercase();
                //LE("%s:%s",LCSTR(tagns),LCSTR(tagname));
                ...
                if (tagname == "td" && close_flag)
                {
                    ...
                    if(flag_colspan)
                    {
                        //LE("flag_colspan, val = %s ",LCSTR(colspan_val));
                ...
                if (in_rpr)
                {
                    ...
                    if (tagname == "b")
                    {
                        //LE(" b closeflag = %d",close_flag?1:0);
                ...
                {
                    callback_->OnTagClose(tagns.c_str(), tagname.c_str());
                    //LV("</%s:%s>", LCSTR(tagns),LCSTR(tagname));
                    if (SkipTillChar('>'))
                    {
                        m_state = ps_text;
                        ch = ReadCharFromBuffer();
            ...
            case ps_attr: //read tags
            {
                ...
                if(in_imagedata && attrname == "id")
                {
                    in_imagedata = false;
                    attrname = "src";
                    attrvalue = callback_->convertId(docxLinks.findTargetById(attrvalue));
                }
                ...
                if(in_tcpr)
                {
                    if(tagname == "gridspan" && attrname == "val")
                    {
                        ...
                        //LE("in_gridspan , [val] = [%s]",LCSTR(colspan_val));
                        flag_colspan = true;
                    }
                }
                ...
                if(in_pstyle)
                {
                    ...
                    {
                        in_header = true;
                        if(headermap.find(attrvalue)!=headermap.end())
                        {
                            pstyle_value = headermap[attrvalue];
                            //LE("pstyle = %d",pstyle_value);
                        }
                        else
                        {
                ...
                if(in_footnoteref)
                {
                    ...
                    //LE("linksmap add = %s %s", LCSTR(href),LCSTR(id));
                    attrvalue = href;
                    ...
                    if(attrname == "id")
                    {
                        ...
                        if(LinksMap.size() !=0)
                        {
                            if(LinksMap.find((lString16("#") + attrvalue).getHash())!=LinksMap.end())
                            {
                                ...
                                //LE("found [%s] at [%s]",LCSTR(currlink_id),LCSTR(temp_section_id));
                ...
                if(!noattrib)
                {
                    //LE("OnAttrib [%s:%s = \"%s\"]",LCSTR(attrns), LCSTR(attrname), LCSTR(attrvalue));
                    callback_->OnAttribute(attrns.c_str(), attrname.c_str(), attrvalue.c_str());
                }
                if(separate_img)
                {
...
void LvXmlParser::initOdtTagsFilter(){
    ...
    tags.clear();
    tags_init_ = true;
    LV("ODT tag filtering array initialized");
    return;
}
        ...
        switch (m_state)
        {
            ...
            case ps_lt:  //look for tags
            {
                ...
                if((tagname=="h" && close_flag) || (tagname=="h" && !close_flag && in_h))
                {
                    ...
                    {
                        //LE("hlevel_backup close = %d", hlevel_backup);
                ...
                if((tagname=="style" || tagname=="list-style") && close_flag)
                {
                    ...
                    if(currstyle.valid)
                    {
                        LE("styles add %s",LCSTR(currstyle.name_));
            ...
            case ps_attr: //read tags
            {
                ...
                if (in_h)
                {
                    ...
                    if (attrname == "outline-level")
                    {
                        ...
                        //LE("hlevel_backup  = [%d]", hlevel_backup);
                    }

                    if (attrname == "style-name")
                    {
                        hlevel = odtStyles.GetHeaderById(attrvalue);
                        //LE("hlevel  [%s] => [%d]", LCSTR(attrvalue), hlevel);
                    }
                }
            ...
            case ps_text:
            {
                ...
                {
                    //LE("adding hidden %d %d",in_noteref,in_notebody);
                    callback_->OnAttribute(L"",L"class",L"hidden");
                }
                ...
                if(in_h)
                {
                    ...
                    {
                        //LE("!in h printed");
                        //LE("tagname = %s",LCSTR(tagname));
                        in_h_printed = true;
                        if (hlevel != 0)
                        {
                            //LE("hlevel open = %d", hlevel);
                            switch (hlevel)
                            {
                        ...
                        {
                            //LE("hlevel_backup open = %d", hlevel_backup);
                            switch (hlevel_backup)
                            {
                                case 1: callback_->OnTagOpen(L"", L"h1"); break;
                                case 2: callback_->OnTagOpen(L"", L"h2"); break;
    ...
    for (; !eof_ && !error && !firstpage_thumb_num_reached ;)
    {
        ...
        switch (m_state)
        {
            ...
            case ps_lt:  //look for tags
            {
                ...
                tagns.lowercase();
                tagname.lowercase();
                //if(!close_flag) LE(" <%s>",LCSTR(tagname));
                //if( close_flag) LE("</%s>",LCSTR(tagname));
            ...
            case ps_attr: //read tags
            {
                ...
                if((tagname == "section" || in_section ) && tagname != "title"  && attrname == "id")
                {
                    ...
                    //LE("temp_section_id = [%s]",LCSTR(temp_section_id));
                }
            ...
            case ps_text:
            {
                ...
                if (save_title_content)
                {
                    ...
                    //LE("saving to buffer = [%s]", LCSTR(buffer));
                    if (buffer.atoi() <= 0)
                    {
                        buffernum = -1;
                        title_is_text = true;
...
//#endif
//    	LV("Buf:'%s'", LCSTR(lString16(m_read_buffer, m_read_buffer_len)) );
...
{
    lString16 temp;
    return this->ReadTextToString(temp,true,true);
    temp.clear();
}
...
lString16 htmlCharset(lString16 htmlHeader)
{
    ...
    //LV("http-equiv content-type: %s", UnicodeToUtf8(htmlHeader).c_str());
