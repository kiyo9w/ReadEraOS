...
lString16 GetFb3CoverImagePath(LVContainerRef container)
{
    ...
    LE("GetFb3CoverImagePath: no cover found");
...
bool ImportFb3Document(LVStreamRef in_stream, CrDom *m_doc, bool firstpage_thumb)
{
    ...
    if (arc.isNull())
    {
        LE("This FB3 is corrupted: not a ZIP archive");
    ...
    if (decryptor->open())
    {
        LD("Fb3: encrypted items detected");
        return false;
    }
    ...
    {
        LE("ImportFb3Document : failed to open rootfile %s ", LCSTR(rootfilePath));
        LE("ImportFb3Document : Retrying...");
        ...
        if (dom)
        {
            for (int i = 1; i < 500; i++)
            {
                ...
                if(item->getAttributeValue("ContentType") == "application/fb3-body+xml")
                {
                    ...
                    if (content_stream.isNull())
                    {
                        LE("ImportFb3Document : Retry failed!");
                        return false;
                    }
                    else
                    {
                        LE("ImportFb3Document : Retry succeeded!!");
                    }
                }
            }
        }
    ...
    if (!content_stream.isNull())
    {
        ...
        if (!dom)
        {
            LE("processMeta: malformed FB3 (2)");
        }
        else
        {
    ...
    if (!stream.isNull())
    {
        ...
        {
            LE("Unable to parse fb3 file at [%s]", LCSTR(rootfilePath));
        }
    }
        ...
        lString16 *res_series, int *res_series_number, lString16 *res_genre, lString16 *res_annotation)
{
    ...
    *res_authors = metaSanitize(authors);
    ...
    *res_genre = metaSanitize(genre);


    *res_title      = metaSanitize(dom->textFromXPath(lString16("fb3-description/title/main")).trim());
    *res_title      += " " + metaSanitize(dom->textFromXPath(lString16("fb3-description/title/sub")).trim());
    *res_lang       = metaSanitize(dom->textFromXPath(lString16("fb3-description/lang")).trim());
    *res_annotation = metaSanitize(dom->textFromXPath(lString16("fb3-description/annotation")).trim());

    *res_series = metaSanitize(series); // empty
...
LVStreamRef GetFb3CoverImage(LVContainerRef container)
{
    ...
    if (result.isNull())
    {
        LE("GetFb3CoverImage: no cover found");
        return LVStreamRef();
    }
    return result;
}
            ...
            for (int i = 1; i < 5000; i++)
            {
                ...
                if(type.pos("image")!=-1 || type.pos("thumbnail")!=-1)
                {
                    for (int i = 0; i < names.length(); i++)
                    {
                        ...
                        {
                            //LE("id = [%s] old target = [%s] target = [%s]", LCSTR(id), LCSTR(target), LCSTR(names.get(i)));
