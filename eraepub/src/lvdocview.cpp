...
#if 0
#define REQUEST_RENDER(caller) { LV("RequestRender " caller); RequestRender(); }
#define CHECK_RENDER(caller) { LV("CheckRender " caller); CheckRender(); }
#else
...
void LVDocView::RenderIfDirty()
{
    ...
    if (cr_dom_ && cr_dom_->getRootNode() != NULL)
    {
        ...
        {
            LE("RenderIfDirty base_font_.isNull()");
            return;
        }
...
{
    cachedPage = -2;
    cachedPageText = lString16::empty_str;
    is_rendered_ = false;
...
void LVDocView::Resize(int width, int height)
{
    ...
    {
        LV("Size is not changed: %dx%d", width, height);
        return;
    }
    width_ = width;
    height_ = height;
...
LVStreamRef LVDocView::OreResolveStream(int format, const char *path, int fd, uint32_t direct_archive)
{
    ...
    if (stream.isNull())
    {
        LE("OreResolveStream open file fail %s", LCSTR(absolute_path));
        return LVStreamRef();
    }
    if (OreIsNormalDirectArchive(direct_archive))
    {
        ...
        if (container.isNull())
        {
            LE("OreResolveStream read archive fail %s", LCSTR(absolute_path));
            return LVStreamRef();
        }
        stream = container->OpenStreamByPackedSize(direct_archive);
        if (stream.isNull())
        {
            LE("OreResolveStream direct archive stream fail %s", LCSTR(absolute_path));
            return LVStreamRef();
        }
    }
    ...
    if (container.isNull())
    {
        LE("OreResolveStream smart_archive fail %s", LCSTR(absolute_path));
        return LVStreamRef();
    }
    int found_count = 0;
    ...
    {
        LE("OreResolveStream smart_archive collision fail %s %d",
...
bool LVDocView::LoadDoc(int doc_format, const char *path, int fd, uint32_t direct_archive)
{
    ...
    {
        LV("LoadDoc: Partial parsing: %s %d", LCSTR(lString16(path)), doc_format);
    }
    else
    {
        LV("LoadDoc: Full parsing: %s %d", LCSTR(lString16(path)), doc_format);
    }
#endif
    LVStreamRef stream;
    if (fd > 0)
...
bool LVDocView::LoadDoc(int doc_format, LVStreamRef stream)
{
    ...
    if (parser)
    {
        ...
        {
            LW("Image found in shortened tree. Regenerating full tree.");
            cr_dom_->clear();
            parser->FullDom();
            if (!parser->Parse())
            {
...
void LVDocView::DrawPageTo(LVDrawBuf *drawbuf, LVRendPageInfo &page, lvRect *pageRect)
{
    ...
    {
        base_font_ = fontMan->GetFont(cfg_font_size_, 400, false, DEF_FONT_FAMILY, cfg_font_face_);
        int japaneseDotWidth = base_font_->getCharWidth(L'。', '?');
        clip.top   -= japaneseDotWidth / 4;
        clip.right += japaneseDotWidth;
    }
    ...
    {
        LV("Entering DrawDocument(): %d ranges", marked_ranges_.length());
    }
    if (page.height)
    {
...
{
    //LV("LVDocView::GoToPage START page=%d, page_=%d", page, page_);
    ...
    //LV("LVDocView::GoToPage END page=%d, page_=%d", page, page_);
    return res;
}
...
bool LVDocView::WindowToDocPoint(lvPoint &pt)
{
    ...
    else
    {
        ...
        if (rc && page >= 0 && page < pages_list_.length())
        {
            ...
            {
                //LD(" point page offset( %d, %d )", pt.x, pt.y );
                pt.y += page_y;
                return true;
            }
        }
...
bool LVDocView::DocToWindowRect(lvRect &rect, bool modify)
{
    ...
    {
        //LE("NOT PASSED BY VERTICAL CHECKS. Index = %d",index);
        //LE("rect.top+1 <= (pages_list_[page]->start + pages_list_[page]->height)");
        //LE("%d <= (%d )",rect.top+1, pages_list_[page]->start + pages_list_[page]->height);
        return false; // went out of bounds of pages array
    }
...
LVImageSourceRef LVDocView::getImageByPoint(lvPoint pt)
{
    ...
    if (!res.isNull())
    {
        LD("getImageByPoint(%d, %d): found image %d x %d",
...
void LVDocView::UpdateSelections()
{
    ...
    //LV("updateSelections() : selection count = %d", cr_dom_->getSelections().length());
...
LVRef<ldomXRange> LVDocView::GetPageDocRange(int page_index , bool one_column)
{
    ...
    else
    {
        ...
        {
            LE("Fatal error: no pages in pages list found!");
            return res;
        }
        LVRendPageInfo *page = pages_list_[page_index];
        if (page->type != PAGE_TYPE_NORMAL)
        {
            //LE("Fatal error: page->type != PAGE_TYPE_NORMAL");
            return res;
        }
        ...
        if (start.isNull() || end.isNull())
        {
            //LE("Fatal error: start.isNull() = %d || end.isNull() = %d", start.isNull(), end.isNull());
            return res;
        }
        res = LVRef<ldomXRange>(new ldomXRange(start, end));
    }
...
LVArray<TextRect> LVDocView::GetPageFootnotesText(int page, bool rightpage)
{
    ...
    if (fnotes.empty())
    {
        //LV("No footnotes on page %d",page);
        return result;
    }
    ...
    //LE("xp1 node = %s",LCSTR(xp1.getNode()->getXPath()));
    //LE("xp2 node = %s",LCSTR(xp2.getNode()->getXPath()));
    ...
    if (word_chars.empty())
    {
        LE("No footnotes rects on current page, but footnotes exist!");
        return result;
    }
...
//		LE("before rect = [%d:%d][%d:%d]",rect.left,rect.right,rect.top,rect.bottom);
        if (rect.top != abs_top)
        {
...
void LVDocView::GetCurrentPageLinks(LVArray<TextRect>& links_list)
{
    ...
    class LinkKeeper : public ldomNodeCallback
    {
        ...
        {
            //LE("final node = %s",LCSTR(node->getXPath()));
        ...
        void ProcessImageNode(ldomNode *node)
        {
            ...
            //LE("in node = %s", LCSTR(node->getNodeName()));
    ...
    public:
        ...
        virtual void onText(ldomXRange *node_range)
        {
            ...
            LD("GetCurrentPageLinks first text on page: %d-%d %s",
        ...
        {
            if (node->getNodeId() != el_a && node->getParentNode("a") == NULL)
            {
                return true;
            }
#ifdef OREDEBUG
            if (node->getChildCount() == 0)
            {
                ...
                LV("GetCurrentPageLinks empty link in malformed doc");
            }
#endif
        ...
        //in foreach2:
        //endfound = callback->processElement
        bool processElement(ldomNode* node, ldomXRange * range)
        {
            for (lUInt32 i = 0; i < node->getChildCount(); i++)
            {
                ...
                if (child->isText())
                {
                    //LE("_______TEXT nodepath = %s",LCSTR(child->getXPath()));
...
LVArray<lvRect> LVDocView::GetCurrentPageParas()
{
    ...
    class ParaKeeper : public ldomNodeCallback
    {
        ...
        {
            //LE("in node = %s",LCSTR(node->getXPath()));
            lvRect paraend;
            ...
            if (childcount == 0 && node->isNodeName("br"))
            {
                ...
                if (TxtNode == NULL)
                {
                    //LE("prevnode = [%s]",LCSTR(prevNode->getXPath()));
                    //LE("BR3");
                    return paraend;
                }
                //LE("BR OK");
            ...
            for (lUInt32 i = 0; i < childcount; i++)
            {
                ...
                if (child->isText())
                {
                    ...
                    //  LE("Final node processed");
                    //  LE("final node = %s",LCSTR(child->getNodeName()));
                    //  LE("final text = %s",LCSTR(child->getText()));
                }
                else
                {
                    //    LE("diving deeper");
                ...
                if (child == range->getEndNode())
                {
                    //LE("endnode found for processnodeparaends");
                    endnode_found_ = true;
                    break;
                }
                if (endnode_found_)
            ...
            if(NodeIsAllowed(node))
            {
                ...
                //   LE("paraend = [%d:%d][%d:%d]",paraend.left,paraend.right,paraend.top,paraend.bottom);
            }
            return paraend;
        }
        ...
        lvRect ProcessFinalNode_GetNodeEnd(ldomNode *node)
        {
            ...
            //debug test: New getrect vs old getrect
            {
                ...
                if(oldrect!=newrect)
                {
                    LW("new rect != old rect [%d:%d][%d:%d] != [%d:%d][%d:%d]",newrect.left,newrect.right,newrect.top,newrect.bottom,oldrect.left,oldrect.right,oldrect.top,oldrect.bottom);
                }
            }
#endif
            //old implementation
            ...
            {
                LW("Unable to get node end coordinates. Ignoring");
                //para_rect_array.add(empty_rect);
                return empty_rect;
            }
            //LE("node = %s",LCSTR(node->getNodeName()));
            //LE("text = %s",LCSTR(node->getText()));
            //LE("rect = [%d:%d][%d:%d]",end_rect.left,end_rect.right,end_rect.top,end_rect.bottom);
...
void LVDocView::GetOutline(LVPtrVector<LvTocItem, false> &outline)
{
    ...
    if (cr_dom_)
    {
        ...
        {
            LE("outline root childcount = 0, no table of contents generated");
        }
        else
        {
...
float LVDocView::CalcRightSide(TextRect textrect)
{
    ...
    //LE("cfg  margins [%d:%d][%d:%d]",page_margins.left,page_margins.right,page_margins.top,page_margins.bottom);
    //LE("node margins [%d:%d][%d:%d]",nodemargins.left,nodemargins.right,nodemargins.top,nodemargins.bottom);
...
LVArray<lvRect> LVDocView::GetPageParaEnds()
{
    ...
    for (int i = 0; i < raw_para_array.length(); i++)
    {
        ...
        //LE("raw_pararect = [%d:%d][%d:%d]",rawrect.left,rawrect.right,rawrect.top,rawrect.bottom);
        ...
        //LE("pararect: [%d:%d] [%d:%d]", rawrect.left, rawrect.right, rawrect.top-offset, rawrect.bottom-offset);
        result.add(rawrect);
    }
...
std::vector<lString16> getAllImagesXpathsRecursive(ldomNode* node)
{
    ...
    for (int i = 0; i < node->getChildCount(); i++)
    {
        ...
        if(child->isImage())
        {
            result.emplace_back(child->getXPath());
        }
        //element node
        std::vector<lString16> nodelist = getAllImagesXpathsRecursive(child);
        result.insert(result.end(),nodelist.begin(),nodelist.end());
    }
    return result;
};

std::vector<lString16> LVDocView::getAllImagesXpaths()
{
    return getAllImagesXpathsRecursive(cr_dom_->getRootNode());
}
...
void printLongMsg(lString16 input)
{
    ...
    if(count>1)
    {
        ...
        for (int i = 0; i < count+1; i++)
        {
            ...
            LE("frag %d = [%s]",i,LCSTR(print));
            start +=500;
        }
    }
    else
    {
        LE("input = [%s]",LCSTR(input));
    }
}
#endif
...
SearchResult LVDocView::FindAndTrimNextPage(lString16 base1, lString16 query, int page, int start_last)
{
    ...
    while (pos > -1)
    {
        ...
        if (qtemp == ttemp)
        {
            //LE("qtemp = [0:%d] {%s}, ttemp = [%d:%d] {%s}",tlen1-firstcharpos,LCSTR(qtemp),firstcharpos,tlen1-firstcharpos,LCSTR(ttemp));
            //LE("qlen1 = %d, pos = %d",qlen1,pos);
            qlen1 = tlen1 - pos;
            break;
        }
    ...
    //LE("lastIndex PREVIEWS [%d] = [%d-%d-1]",lastIndex,qlen,qlen1);
...
LVArray<TextRect> LVDocView::GetSearchHitboxes(LVArray<ldomWord> words_list, lString16 query, bool wordBeginFlag)
{
    ...
    bool shortQuery = qlen < 3;
    bool query_C_or_J = query.checkCJ_bool();
    bool query_K = query.checkK_bool();
    ...
    while (pos<words_list.length())
    {
        ...
        if (wordBeginFlag && pos > 0 && !query_C_or_J )
        {
            ldomWord word = words_list.get(pos);
            if (word.isNull() || word.getNode()->isNull())
            {
                continue;
            }
            ...
            if(shortQuery && !query_K && pos + qlen < words_list.length())
            {
                ldomWord after_word = words_list.get(pos+qlen);
                if (after_word.isNull() || after_word.getNode()->isNull())
                {
                    continue;
                }
                rectHelper.Init(after_word.getNode());
                lvRect pre_rect = rectHelper.getRect(after_word,true);

                lChar16 ch_after = after_word.getCachedText();

                if (!UnicodeHelper::isPunct(ch_after) && !UnicodeHelper::isSpace(ch_after))
                {
                    pos += query.length();
                    continue;
                }
            ...
            ldomWord pre_word = words_list.get(pos-1);
            if (pre_word.isNull() || pre_word.getNode()->isNull())
            {
                continue;
            }
            rectHelper.Init(pre_word.getNode());
            lvRect pre_rect = rectHelper.getRect(pre_word,true);

            lChar16 ch_before = pre_word.getCachedText();

            if (!UnicodeHelper::isPunct(ch_before) &&
                !UnicodeHelper::isSpace(ch_before) &&
                rect.top == pre_rect.top &&
                rect.bottom == pre_rect.bottom)
            {
                pos += query.length();
                continue;
            }
...
std::vector<Hitbox> LVDocView::unionRects(std::vector<Hitbox> rects)
{
...
LVArray<Hitbox> RTL_Wrap(LVArray<Hitbox> rects)
{
    LVArray<Hitbox> result;
    for (int i = 0; i < rects.size(); i++)
    {
        Hitbox curr = rects.at(i);
        Hitbox smallFirst = curr;
        Hitbox smallLast = curr;

        // making left == right for selection marks to work properly
        smallFirst.left_ = smallFirst.right_;
        smallLast.right_ = smallLast.left_;

        // adding those hitboxes for rtl detection // RIGHT FIRST BECAUSE ITS RTL
        if (smallLast.getNode()->isRTL())  {result.add(smallLast);}
        result.add(curr);
        if (smallFirst.getNode()->isRTL()) {result.add(smallFirst);}

    }
    return result;
}

LVArray<Hitbox> CJK_Wrap(LVArray<Hitbox> rects)
{
    LVArray<Hitbox> result;
    for (int i = 0; i < rects.size(); i++)
    {
        ...
        smallFirst.right_ = smallFirst.left_;
        smallLast.left_ = smallLast.right_;

        // adding those hitboxes for rtl detection
        result.add(smallFirst);
        result.add(curr);
        result.add(smallLast);
    }
    return result;
}
...
LVArray<TextRect> LVDocView::GetSearchHitboxesPrevPage(LVArray<ldomWord> base1, lString16 query, int last_len)
{
    ...
    //LE("lastIndex HITBOXES [%d] = [%d-%d-1]",lastIndex,qlen,last_len);
...
int LVDocView::GetSearchHitboxesNextPageLite(int page, lString16 query)
{
    ...
    while (pos > -1)
    {
        ...
        if (qtemp == ttemp)
        {
            //LE("qtemp = [0:%d] {%s}, ttemp = [%d:%d] {%s}",tlen-pos,LCSTR(qtemp),pos,tlen-pos,LCSTR(ttemp));
            //LE("tlen = %d qlen1 = %d, pos = %d",tlen,qlen1,pos);
            qlen1 = tlen - pos;
            break;
        }
...
LVArray<TextRect> LVDocView::GetSearchHitboxesNextPage(LVArray<ldomWord> base1, int page, lString16 query)
{
    ...
    while (pos > -1)
    {
        ...
        //LE("qtemp = [0:%d] {%s}, ttemp = [%d:%d] {%s}",tlen1-pos,LCSTR(qtemp),pos,tlen1-pos,LCSTR(ttemp));
        //LE("tlen1 = %d , pos = %d",tlen1,pos);
        if (qtemp == ttemp)
        {
            break;
        }
    ...
    //LE("xp end = %s",LCSTR(xp1.toString()));
    if(xp1.isNull())
    {
        return result;
    }
...
LVArray<Hitbox> LVDocView::SearchForTextHitboxes(int page, lString16 query, bool wordBeginFlag)
{
    this->GoToPage(page);
    LVArray<Hitbox> result;
    ...
    rects.add(GetSearchHitboxes(base, query, wordBeginFlag));
    ...
    if(!firstnode->isNull())
    {
        ...
        if(rectheight < fontheight)
        {
            ...
            //LE("strheight_last A = %d",strheight_last);
        }
        else if( rectheight >= fontheight * coeff && rectheight <= fontheight * coeff * 2 )
        {
            ...
            strheight_last = rectheight;
            //LE("strheight_last B = %d",strheight_last);
        }
        else if (rectheight < fontheight * coeff && rectheight > fontheight)
        {
            strheight_last = rectheight;
            //LE("strheight_last C = %d",strheight_last);
        }
        else
        {
            strheight_last = fontheight;
            //LE("strheight_last D = %d",strheight_last);
        }
    }
    else
    {
        strheight_last = rects.get(0).getRect().height();
        //LE("strheight_last E = %d",strheight_last);
    }
    for (int i = 0; i < rects.length(); i++)
    {
        ...
        //LE("strheight_curr = %d, strheight_last = %d, if = %f",strheight_curr, strheight_last, strheight_last * STRHEIGHT_THRESHOLD);
        if (strheight_curr >= strheight_last * STRHEIGHT_THRESHOLD)
        {
            //LE("STRHEIGHT_THRESHOLD");
        ...
        //LE("hb [%d:%d][%d:%d] [%s]",rect.left,rect.right,rect.top,rect.bottom,LCSTR(word.getText()));
        Hitbox hitbox = Hitbox(l,r,t,b,xpath,word);
        result.add(hitbox);
    }
    result = unionRectsTCheck(result);
    if (gDocumentRTL)
    {
        result = RTL_Wrap(result);
    }
    else if ( gJapaneseVerticalMode)
    {
        result = CJK_Wrap(result);
    }
    return result;
}
...
LVArray<Hitbox> LVDocView::GetPageHitboxesRTL(ldomXRange *in_range, int page)
{
    ...
    hitboxes = hitboxesCacheManager.findPageHitboxes(page);
    if (hitboxes.empty())
    {
        //CRLog::error("Generating new cache for page %d ",page);
        //CRLog::error("old page = %d",hitboxesCache.getPage());
        GoToPage(page);
        hitboxes = GetPageHitboxes();
        hitboxesCacheManager.addItem(PageHitboxesCache(page, hitboxes));
    }
    ...
    {
        LE("failed to get startindex or endindex (%d)(%d)",startIndex,endIndex);
        return result;
    }
...
LVArray<Hitbox> LVDocView::GetPageHitboxes(ldomXRange* in_range, bool rtl_enable, bool rtl_space)
{
	...
	//LE("crexp = %s",LCSTR(xp));
	//LE("text  = %s",LCSTR(text));

	//xp = cr_dom_->convertXPointerToMuPdf(crexp);
	//LE("pdfxp = %s",LCSTR(xp));


	//LV("GETPAGEHITBOXES start");
    LVArray<Hitbox> result;
    float page_width    = this->GetWidth();
    float page_height   = this->GetHeight();
    ...
    if(in_range->isNull())
    {
        ...
        {
            LE("UNABLE TO TAKE PAGE [%d] RANGE ", this->GetCurrPage());
            return result;
        }
    }
    else
    ...
    for (int i = 0; i < word_chars.length(); ++i)
    {
        ...
        while (img_counter < images_array.length())
        {
            ...
            {
                //LE("letter       = %s",LCSTR(text));
                //LE("rect.bottom     = %d",rect.bottom);
                //LE("img_rect.bottom = %d",img_rect.bottom);
        ...
        if (strheight_last != 0 && strheight_curr > strheight_last * STRHEIGHT_THRESHOLD)
        {
            ...
            //LE("linebreak letter = %s",LCSTR(text));
        ...
        { //usual single-line words
            ...
            {
#ifdef OREDEBUG
                //LE("invalid character [%s] %x : too short height!",LCSTR(text),text.firstChar());
#endif
                continue;
            }
            ...
            if(text == " ")
            {
                rect = FillInbetweenSpace(rect, i, &word_chars);
            }
            ...
            //LE("usual letter = %s rect [%f:%f]:[%f:%f]", LCSTR(text),l*page_width,t*page_height,r*page_width,b*page_height);
            //LE("usual letter = %s rect %d;%f:%f:%f:%f", LCSTR(text),page_,l,r,t,b);
...
#endif
#endif //OREDEBUG
    //LV("GETPAGEHITBOXES end");

    return result;
}

lvRect LVDocView::FillInbetweenSpace(lvRect input, int i, LVArray<TextRect> *word_chars)
{
    lvRect rect = input;
    int strheight_curr = rect.height();

    if(word_chars->length() <= i + 1 || i <= 0)
    {
        return input;
    }

    lString16 nexttext = word_chars->get(i + 1).getText();
    lvRect    nextrect = word_chars->get(i + 1).getRect();
    DocToWindowRect(nextrect);

    lString16 lasttext = word_chars->get(i - 1).getText();
    lvRect    lastrect = word_chars->get(i - 1).getRect();
    DocToWindowRect(lastrect);

    bool lastRTL = lasttext.CheckRTL();
    bool nextRTL = nexttext.CheckRTL();

    if(rect.top != nextrect.top || rect.top != lastrect.top )
    {
        return input;
    }

    if(nextrect.height() >= strheight_curr * 1.5)
    {
        return input;
    }

    if(lastrect.height() >= strheight_curr * 1.5)
    {
        return input;
    }
    ...
    if(nextRTL && lastRTL)
    {
        ...
        //LE("pure RTL space : [%s] [ ] [%s] -> l=[%d]  r=[%d]", LCSTR(nexttext), LCSTR(lasttext), rect.left, rect.right);

    }
    else if (!nextRTL && !lastRTL)
    {
        //NON-RTL
        ...
        if (!gDocumentRTL)
        {
            ...
            //LE("NON-RTL pure 0: [%s] [ ] [%s] -> l=[%d]  r=[%d]", LCSTR(lasttext), LCSTR(nexttext), rect.left, rect.right);
        }
        else
        {
            bool nextnode_RTL = word_chars->get(i + 1).getNode()->isRTL();
            bool lastnode_RTL = word_chars->get(i - 1).getNode()->isRTL();

            if (lastnode_RTL && nextnode_RTL)
            {
                // NON-RTL in RTL NODES
                rect.left  = nextrect.right;
                rect.right = lastrect.left;
                //LE("NON-RTL in RTL NODES : [%s] [ ] [%s] -> l=[%d]  r=[%d]", LCSTR(lasttext), LCSTR(nexttext), rect.left, rect.right);
            }
            else if (!lastnode_RTL && !nextnode_RTL)
            {
                // NON-RTL pure
                rect.left  = lastrect.right;
                rect.right = nextrect.left;
                //LE("NON-RTL pure 1: [%s] [ ] [%s] -> l=[%d]  r=[%d]", LCSTR(lasttext), LCSTR(nexttext), rect.left, rect.right);
            }
            else
            {
                //NON-RTL in mixed context
                rect.left  = nextrect.right;
                rect.right = lastrect.left;
                //LE("NON-RTL in mixed context : [%s] [ ] [%s] -> l=[%d]  r=[%d]", LCSTR(lasttext), LCSTR(nexttext), rect.left, rect.right);
            }
        }
    }
    else
    {
        ...
        rect.left  = nextrect.right;
        rect.right = lastrect.left;
        //LE("BIDI space : [%s] [ ] [%s] -> l=[%d]  r=[%d]", LCSTR(nexttext), LCSTR(lasttext), rect.left, rect.right);
    }
...
lString16 LVDocView::GetXpathFromPageById(int page, int id, bool IsEndXpath)
{
    ...
    LVArray<Hitbox> hitboxes = hitboxesCacheManager.findPageHitboxes(page);
    if (hitboxes.empty())
    {
        //CRLog::error("Generating new cache for page %d ",page);
        //CRLog::error("old page = %d",hitboxesCache.getPage());
    ...
    //else
    //{
    //    CRLog::error("Using cache for page %d ",page);
    //}
    ldomWord word;

    if (id < 0 && id >= hitboxes.length())
    {
        //LE("out of range");
        return lString16("-");
    }

    word = hitboxes.get(id).word_;

    if (word.isNull())
    {
        if(IsEndXpath)
        {
            //LE("go back to id = %d",id - 1);
            return GetXpathFromPageById(page, id - 1, IsEndXpath);
        }
        else
        {
            //LE("go forth to id = %d",id + 1);
            return GetXpathFromPageById(page, id + 1, IsEndXpath);
        }
    }
    ...
    if(IsEndXpath)
    {
        //LE("endxpath for word [%s] = %s",LCSTR(word.getText()),LCSTR(word.getEndXPointer().toString()));
        return  word.getEndXPointer().toString();// + coords;
    }
    else
    {
        //LE("startxpath for word [%s] = %s",LCSTR(word.getText()),LCSTR(word.getStartXPointer().toString()));
...
lString16 LVDocView::GetXpathByRectCoords(lString16 key, ldomWordMap m)
{
    ...
    else
    {
        ...
        ///LE("xpointer1 for key [%d__%s] = [%s]",page,LCSTR(key_start),LCSTR(xPointer.toString()));
    }
}
...
std::vector<std::wstring> LVDocView::checkIfPageHasWords(uint32_t pagenum)
{
    lString16 pagetext = GetPageText(pagenum);
    if (pagetext.empty())
    {
        return std::vector<std::wstring>();
    }
    pagetext = pagetext.ReplaceUnusualSpaces();
    pagetext = pagetext.lowercase();

    const std::wstring text = lowercase(std::wstring(pagetext.c_str(),pagetext.length()));

    std::vector<std::wstring> result;
    if( !searchDictionary.empty() )
    {
        std::vector<std::wstring> regular_result = checkTextForDict(text, searchDictionary);
        result.insert(end(result), begin(regular_result), end(regular_result));
    }
    if(!searchDictionaryCJK.empty() && gDocumentCJK)
    {
        std::vector<std::wstring> cjk_result = checkTextForDictSimple(text, searchDictionaryCJK);
        result.insert(end(result), begin(cjk_result), end(cjk_result));
    }
    if(!searchDictionaryRTL.empty() && gDocumentRTL)
    {
        std::vector<std::wstring> rtl_result = checkTextForDict(text, searchDictionaryRTL);
        result.insert(end(result), begin(rtl_result), end(rtl_result));
    }
    if(!searchDictionaryIndic.empty() && gDocumentINDIC)
    {
        std::vector<std::wstring> indic_result = checkTextForDict(text, searchDictionaryIndic);
        result.insert(end(result), begin(indic_result), end(indic_result));
    }
    return result;
}


int LVDocView::doRoughXpathSearch(const lString16& xpath, const LVArray<Hitbox>& hitboxes)
{
    ldomXPointer find_xp  = GetCrDom()->createXPointer(xpath);
    if(find_xp == ldomXPointer())
    {
        //not found
        return -1;
    }
    ldomNode *node = find_xp.getNode();
    if(node == NULL)
    {
        //not found
        return - 1;
    ...
    for(int i = 0; i < hitboxes.size(); i++)
    {
        ...
        //LE("Curr = [%d] [%ls] [%s]",i, hb.text_.c_str(), hb.getXpointer().c_str());
        auto curr_xp = ldomXPointer(hb.getNode(), 0);
        ldomXRange range(find_xp, curr_xp);
        ...
        int lvl = parent->getNodeLevel();
        map[lvl].emplace_back(i);

        //LE("Common parent = [%s]",LCSTR(parent->getXPath()));
    }
    if(map.empty())
    {
        return -1;
    ...
    std::vector<int> deepest_lvl = iterator->second;
    if(deepest_lvl.empty())
    {
        return -1;
    }
    std::sort(deepest_lvl.begin(), deepest_lvl.end());
    return deepest_lvl.front();
}