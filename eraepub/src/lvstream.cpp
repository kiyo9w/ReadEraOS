    ...
    */
    ...
    lverror_t error()
    {
		...
		if ( m_hFile!=NULL ) {
			...
			if ( !CloseHandle(m_hFile) )
				LE("Error while closing file handle");
			m_hFile = NULL;
		}
#else
		if ( m_fd!= -1 ) {
            LV("Closing mapped file %s", UnicodeToUtf8(GetName()).c_str() );
			UnMap();
            close(m_fd);
		}
        m_fd = -1;
	...
	lverror_t Map()
	{
		...
		if ( m_hMap==NULL ) {
            ...
            LE( "LVFileMappedStream::Map() -- Cannot map file to memory, err=%08x, hFile=%08x",
                    err, (lUInt32)m_hFile );
            return error();
		}
		...
		if ( m_map==NULL ) {
            LE( "LVFileMappedStream::Map() -- Cannot map file to memory" );
            return error();
		}
		return LVERR_OK;
#else
	...
	lverror_t UnMap()
	{
		...
		if ( m_map!=NULL ) {
			if ( !UnmapViewOfFile( m_map ) ) {
	            LE("LVFileMappedStream::UnMap() -- Error while unmapping file");
				res = LVERR_FAIL;
			}
			m_map = NULL;
		}
    ...
    virtual lverror_t SetSize( lvsize_t size )
    {
		...
		if ( lseek( m_fd, size-1, SEEK_SET ) == -1 ) {
            LE("LVFileMappedStream::SetSize() -- Seek error");
            return error();
        }
        if ( write(m_fd, "", 1) != 1 ) {
            LE("LVFileMappedStream::SetSize() -- File resize error");
            return error();
        }
#endif
		if ( wasMapped ) {
    ...
    {
    	LV("OpenFile: %s", UnicodeToLocal(fname).c_str());
        m_mode = mode;
        ...
        if (m_hFile == INVALID_HANDLE_VALUE || !m_hFile) {
			...
			if ( (m_hFile == INVALID_HANDLE_VALUE) || (!m_hFile) ) {
                LE("Error opening file %s", UnicodeToUtf8(fname).c_str() );
                m_hFile = NULL;
				// error
				return error();
			}
        ...
        if ( mode == LVOM_APPEND && m_size < minSize ) {
            if ( SetSize( minSize ) != LVERR_OK ) {
                LE( "Cannot set file size for %s", fn8.c_str() );
                return error();
            }
        }
        ...
        if (m_fd == -1) {
            LE( "Error opening file %s for %s, errno=%d, msg=%s",
        ...
        struct stat stat;
        if ( fstat( m_fd, &stat ) ) {
            LE( "Cannot get file size for %s", fn8.c_str() );
            return error();
        }
        ...
        m_map = (lUInt8*)mmap( 0, m_size, mapFlags, MAP_SHARED, m_fd, 0 );
        if ( m_map == MAP_FAILED ) {
            LE( "Cannot map file %s to memory", fn8.c_str() );
            return error();
        }
        return LVERR_OK;
#endif
...
public:
    virtual lverror_t Seek( lvoffset_t offset, lvseek_origin_t origin, lvpos_t * pNewPos )
    {
       ...
       LE("error setting file position to %d (%d)", (int)offset, (int)origin );
       return LVERR_FAIL;
    }
    ...
    lverror_t OpenFile( lString16 fname, lvopen_mode_t mode )
    {
        ...
        if (!file)
        {
        	LV( "!opened file: %s mode: %s", UnicodeToLocal(fname).c_str(), modestr);
            m_mode = LVOM_ERROR;
            return LVERR_FAIL;
        }
        m_file = file;
...
//            LV("calling fsync");
            fsync( m_fd );
//            LV("fsync took %d ms", (int)timer.elapsed());
        }
#endif
        return LVERR_OK;
    }
...
//    }
    ...
    lverror_t OpenFile(int fd, int mode) {
        LV("OpenFile via fd %d",fd);
        mode = mode & LVOM_MASK;
        m_fd = fd;
        struct stat stat;
        if (fstat(m_fd, &stat)) {
            LE("Cannot get file size for fd %d", fd);
            return LVERR_FAIL;
        }
    ...
    lverror_t OpenFile(lString16 fname, int mode) {
    	//LV("lverror_t OpenFile: %s", UnicodeToLocal(fname).c_str());
        mode = mode & LVOM_MASK;
#if defined(_WIN32)
        lUInt32 m = 0;
        lUInt32 s = 0;
        ...
        m_fd = open(fn8.c_str(), flags, (mode_t) 0666);
        if (m_fd == -1) {
            LW("Error opening file %s for %s, errno=%d, msg=%s",
            		fn8.c_str(),
        ...
        if (m_zstream.avail_in < ARC_INBUF_SIZE / 4 && m_inbytesleft > 0)
        {
            ...
            else
            {
                ...
                if ( m_CRC != m_originalCRC ) {
                    LE("ZIP stream '%s': CRC doesn't match", LCSTR(lString16(GetName())) );
                    return -1; // CRC error
                }
            }
        }
...
public:
    ...
    virtual lverror_t Read(void* buf, lvsize_t count, lvsize_t* bytesRead)
    {
        ...
        if (readBytes != (int) count) {
            LV("ZIP stream: %d bytes read instead of %d", (int) readBytes, (int) count);
        }
        if (bytesRead)
            *bytesRead = (lvsize_t) readBytes;
        //LV("%d bytes requested, %d bytes read, %d bytes left", count, readBytes, m_outbytesleft);
        return LVERR_OK;
    }
    virtual lverror_t SetSize(lvsize_t)
    {
    ...
    static LVStream* Create(LVStreamRef stream, lvpos_t pos, lString16 name, lUInt32 srcPackSize, lUInt32 srcUnpSize)
    {
        ...
        if(packSize == 0 )
        {
            LW("LVZipDecodeStream::Pack size is 0, setting it to unpSize ");
            packSize = unpSize;
        }
...
public:
    ...
    virtual LVStreamRef OpenStreamByPackedSize(uint32_t size)
    {
        ...
        if (found_count == 0 || found_count > 1) {
            LE("OpenStreamByPackedSize found_count=%d", found_count);
            return LVStreamRef();
        }
        LVStreamRef strm = m_stream; // fix strange arm-linux-g++ bug
...
//                    arcComment=true;
            ...
            if ( ZipHeader.NameLen>max_NM ) {
                LE("ZIP entry name length is too big: %d", (int)ZipHeader.NameLen);
                return 0;
            }
            ...
            if (ReadSize!=SizeToRead) {
                LE("error while reading zip entry name");
                return 0;
            }
...
//#define DUMP_ZIP_HEADERS
            ...
            LV("ZIP entry %s unpSz=%d, pSz=%d, m=%x, offs=%x, zAttr=%x, flg=%x, addL=%d, commL=%d, dn=%d",
...
public:
    ...
    */
	...
	lverror_t CreateCopy( LVStreamRef srcStream, lvopen_mode_t mode )
	{
        ...
        {
            LE("CreateCopy error: stream size is not valid!");
            return LVERR_FAIL;
        }
		m_bufsize = sz;
		m_size = 0;
...
{
    LV("LVCreateDirectory(%s)", UnicodeToUtf8(path).c_str() );
    //LVRemovePathDelimiter(path);
    if ( path.length() <= 1 )
        return false;
    LVContainerRef dir = LVOpenDirectory( path.c_str() );
    if ( dir.isNull() ) {
        LV("Directory %s not found", UnicodeToUtf8(path).c_str());
        LVRemovePathDelimiter(path);
        lString16 basedir = LVExtractPath( path );
        LV("Checking base directory %s", UnicodeToUtf8(basedir).c_str());
        if ( !LVCreateDirectory( basedir ) ) {
            LE("Failed to create directory %s", UnicodeToUtf8(basedir).c_str());
            return false;
        }
#ifdef _WIN32
        ...
        LV("Creating directory %s", path8.c_str() );
        if ( mkdir(path8.c_str(), S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH) ) {
            LE("Cannot create directory %s", path8.c_str() );
            return false;
        }
        return true;
#endif
    }
    LV("Directory %s exists", UnicodeToUtf8(path).c_str());
    return true;
}
