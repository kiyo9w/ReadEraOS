...
void CreBridge::processMeta(CmdRequest &request, CmdResponse &response) {
    ...
    {
        LE("processMeta: FB3  meta extraction");
        LVContainerRef container = LVOpenArchive(stream);
        if (container.isNull()) {
            LE("processMeta: FB3 is not in ZIP");
            response.result = RES_INTERNAL_ERROR;
            return;
        }
        auto decryptor = new EncryptedDataContainer(container);
        if (decryptor->open()) {
            LD("processMeta: FB3 encrypted items detected");
        }
        ...
        if (content_stream.isNull()) {
            LE("processMeta: malformed FB3 (1)");
            response.result = RES_INTERNAL_ERROR;
            return;
        }
    ...
    else if(format == DOC_FORMAT_EPUB)
    {
        LVContainerRef container = LVOpenArchive(stream);
        if (container.isNull()) {
            LE("processMeta: EPUB is not in ZIP");
            response.result = RES_INTERNAL_ERROR;
            return;
        }
        ...
        if (root_file_path.empty()) {
            LE("processMeta: malformed EPUB (0)");
            response.result = RES_INTERNAL_ERROR;
            return;
        }
        auto decryptor = new EncryptedDataContainer(container);
        if (decryptor->open()) {
            LD("processMeta: EPUB encrypted items detected");
        }
        container = LVContainerRef(decryptor);
        lString16 code_base = LVExtractPath(root_file_path, false);
        LVStreamRef content_stream = container->OpenStream(root_file_path.c_str(), LVOM_READ);
        if (content_stream.isNull()) {
            LE("processMeta: malformed EPUB (1)");
            response.result = RES_INTERNAL_ERROR;
            return;
        }
        CrDom *dom = LVParseXMLStream(content_stream);
        if (!dom) {
            LE("processMeta: malformed EPUB (2)");
            response.result = RES_INTERNAL_ERROR;
            return;
        }
    ...
    else if (format == DOC_FORMAT_FB2)
    {
        ...
        if (parser.CheckFormat() && parser.Parse()) {
            ...
            //LE("coverimage extracted == %s", LCSTR(coverimage));
        } else {
            LE("processMeta: !parser.CheckFormat() || !parser.Parse()");
            response.result = RES_INTERNAL_ERROR;
            return;
        }
#ifdef OREDEBUG
