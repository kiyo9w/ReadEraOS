...
#define DEBUG_TEXT
    ...
    if (!iter.isValid())
    {
        LE("processPageText bad request data");
...
#ifdef DEBUG_TEXT
    LD("processPageText external_page=%d page=%d page_width=%d page_height=%d",
    ...
    for (int i = 0; i < hitboxes.length(); i++)
    {
        ...
        responseAddString(response,lString16( currHitbox.text_.c_str()).restoreIndicText());
    }
#undef DEBUG_TEXT
}
...
void CreBridge::processRtlText(CmdRequest &request, CmdResponse &response)
{
    ...
    if (!iter.isValid())
    {
        LE("processPageXpaths bad request data");
        response.result = RES_BAD_REQ_DATA;
        return;
    }
    ...
    //LE("processRtlText in = [%s]",LCSTR(result));

    result = result.ReversePrettyLetters();
    result = result.restoreIndicText();
    //LE("processRtlText out = [%s]",LCSTR(result));

    //LE("str = [%s]",LCSTR(result.processOriyaText()));
...
void CreBridge::processPageRangeText(CmdRequest &request, CmdResponse &response)
{
    ...
    if (!iter.isValid())
    {
        LE("processPageRangeText bad request data : invalid iterator");
        response.result = RES_BAD_REQ_DATA;
        return;
    }
    const char *val = reinterpret_cast<const char *>(temp_val);
    ...
    {
        LE("processPageRangeText: External page %d < 0 OR %d >= %d",
    ...
    //LE("processPageRangeText key_input = %s",LCSTR(key_input));
    ...
    {
        LE("processPageRangeTextSelection out of range : page (%d) < startpage(%d) OR page(%d) > endpage(%d)",
                     page,startpage,page,endpage);
        return;
    }
    //LE("processPageRangeText page (%d), startpage(%d), endpage(%d)",page,startpage,endpage);
    ...
    //LE("processPageRangeText range = %s : %s",LCSTR(range->getStart().toString()),LCSTR(range->getEnd().toString()));
    LVArray<Hitbox> FilteredHitboxes;
    if (gDocumentRTL)
    {
        FilteredHitboxes = doc_view_->GetPageHitboxesRTL(range, page);
        if (FilteredHitboxes.empty())
        {
            LE("processPageRangeText FAILED getting rtl hitboxes for range \n[\n%s \n -> \n%s\n] on page [%d]",LCSTR(range->getStart().toString()),LCSTR(range->getEnd().toString()), page);
            return;
        }
...
void CreBridge::processPageByXPath(CmdRequest& request, CmdResponse& response)
{
    ...
    if (!iter.isValid()) {
        LE("processPageByXPath invalid iterator");
        response.result = RES_BAD_REQ_DATA;
        return;
    }
    ...
    if (bm.isNull()) {
        LE("processPageByXPath bad xpath bm.isNull()");
        response.result = RES_BAD_REQ_DATA;
        return;
    }
    ...
    if (current_page < 0) {
        LE("processPageByXPath bad xpath current_page < 0");
        response.result = RES_BAD_REQ_DATA;
        return;
    }
...
void CreBridge::processPageByXPathMultiple(CmdRequest& request, CmdResponse& response)
{
    ...
    if (!iter.isValid()) {
        LE("processPageByXPathMultiple invalid iterator");
        response.result = RES_BAD_REQ_DATA;
        return;
    }
    ...
    for (int i = 0; i < paths.length(); i++)
    {
        ...
        {
            pages_str += L"-1;";
            LE("processPageByXPathMultiple bad xpath bm.isNull()");
            continue;
        }
        doc_view_->GoToBookmark(bm);
        int current_page = doc_view_->GetCurrPage();
        ...
        {
            pages_str += L"-1;";
            LE("processPageByXPathMultiple bad xpath current_page < 0");
            continue;
        }
    ...
    if (!pages_str.empty())
    {
        ...
        //LE("pages_str = %s",LCSTR(pages_str));
