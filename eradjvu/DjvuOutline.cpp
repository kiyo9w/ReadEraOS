...
void DjvuOutlineItem::toResponse(CmdResponse& response)
{
    ...
    {
        response.addIpcString(EMPTY_TITLE, false);
    }
    else
    {
        std::string str(this->title);
        std::wstring wstr = djvu_stringToWstring(str);
        std::string utf8 = wstring_to_utf8(wstr);

        unsigned int size = (utf8.length() * sizeof(char) * 4) + 1;
        auto chars = (char *) malloc(size);
        memset(chars, 0, size);
        memcpy(chars, utf8.c_str(), utf8.length());
        //LE("chars = [%d] [%s]",size, chars);
        response.addIpcString(chars, true);
        free(chars);
    }
