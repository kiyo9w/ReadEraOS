...
void DjvuBridge::processSmartColontitles(CmdRequest& request, CmdResponse& response)
{
    ...
    calcBitmapColontitleBounds(smart_crop, (uint8_t *) pixels, new_w, new_h, slice_l, slice_t, slice_r, slice_b);
    response.addFloatArray(4, smart_crop, true);
...
std::vector<Hitbox_st> DjvuBridge::GetHitboxesBetweenXpaths(uint32_t page, std::string xpStart, std::string xpEnd, int startPage)
{
...
void DjvuBridge::processPageRangeText(CmdRequest &request, CmdResponse &response) {
    ...
    std::vector<Hitbox_st> BookmarkHitboxes = GetHitboxesBetweenXpaths(page, startstr, endstr, startPage);
...
void DjvuBridge::processRegisterColontitles(CmdRequest &request, CmdResponse &response)
{
