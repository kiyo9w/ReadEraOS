...
std::vector<Hitbox_st> djvu_get_djvu_words_to_array(miniexp_t expr, ddjvu_pageinfo_t *pi, int mode)
{
    ...
    while (miniexp_consp(expr))
    {
        ...
        if (miniexp_stringp(head))
        {
            ...
            for (int i = 0; i < charnum ; ++i)
            {
                ...
                Hitbox_st hb = Hitbox_st(l_,r_,t_,b_,ch,xpath);
                result.push_back(hb);
            ...
            if (mode == PAGETEXT_MODE_WORD_INT)
            {
                ...
                Hitbox_st hb = Hitbox_st(l, r, t_, b_, space, xpath);
                result.push_back(hb);
            }
            //LDD(LOG, "DjvuText: processText: %d, %d, %d, %d: %s", coords[0], coords[1], coords[2], coords[3], text);
            expnum++;
        ...
        {
            std::vector<Hitbox_st> temp = djvu_get_djvu_words_to_array(head, pi, mode);
...
std::vector<Hitbox_st> DjvuBridge::retryProcessTextToArrayCharMode(int pageNo)
{
    pagenum = pageNo;
    ddjvu_pageinfo_t *pi = getPageInfo(pageNo);
    if (pi == NULL)
...
std::vector<Hitbox_st> DjvuBridge::processTextToArray(int pageNo)
{
    pagenum = pageNo;
    ddjvu_pageinfo_t *pi = getPageInfo(pageNo);
    if (pi == NULL)
    ...
    expnum = 0;
    std::vector<Hitbox_st> result = djvu_get_djvu_words_to_array(r, pi, PAGETEXT_MODE_WORD_INT);
...
std::string DjvuBridge::GetXpathFromPageById(std::vector<Hitbox_st> hitboxes, int id, bool addcoords)
{
    std::wstring slashn;
    slashn += '\n';
...
std::vector<std::string> DjvuBridge::GetXpathFromPageById(std::vector<Hitbox_st> hitboxes, std::vector<int> pos_arr, bool addcoords, int qlen)
{
