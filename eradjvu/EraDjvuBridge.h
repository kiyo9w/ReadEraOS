...
private:
    ...
    std::wstring cachedPageText = std::wstring();
    int cachedPageNum = -1;
...
protected:
    ...
    std::vector<Hitbox_st> processTextToArray(int pageNo);
    std::vector<Hitbox_st> retryProcessTextToArrayCharMode(int pageNo);

    std::string GetXpathFromPageById(int page, int id, bool addcoords);
    std::string GetXpathFromPageById(std::vector<Hitbox_st> Hitboxes, int id, bool addcoords);
    ...
    std::vector<Hitbox_st> GetHitboxesBetweenXpaths(uint32_t page, std::string basic_string, std::string basicString, int startPage);
