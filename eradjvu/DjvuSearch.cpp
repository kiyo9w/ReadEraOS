...
std::vector<Hitbox_st> DjvuBridge::SearchForTextHitboxes(int page, std::wstring query, bool wordBeginFlag)
{
...
std::vector<Hitbox_st> DjvuBridge::GetSearchHitboxes(std::vector<Hitbox_st> hitboxes, int page, std::wstring query, bool wordBeginFlag)
{
    std::vector<Hitbox_st> result;
...
std::vector<std::wstring> DjvuBridge::checkIfPageHasWords(uint32_t pagenum)
{
    if(searchDictionary.empty())
    {
        return std::vector<std::wstring>();
    }
    std::wstring pagetext = getPageText(pagenum);
    if(pagetext.length()<=0)
    {
        return std::vector<std::wstring>();
    }
    pagetext = ReplaceUnusualSpaces(pagetext);
    pagetext = lowercase(pagetext);

    std::vector<std::wstring> result = checkTextForDict(pagetext, searchDictionary);
    if(!searchDictionaryCJK.empty())
    {
        if(UnicodeHelper::checkCJ(pagetext))
        {
            std::vector<std::wstring> cjk_result = checkTextForDictSimple(pagetext, searchDictionaryCJK);
            result.insert(end(result), begin(cjk_result), end(cjk_result));
        }
    }
    if(!searchDictionaryRTL.empty())
    {
        if(UnicodeHelper::checkRTL(pagetext))
        {
            std::vector<std::wstring> rtl_result = checkTextForDict(pagetext, searchDictionaryRTL);
...
void DjvuBridge::processRegisterDictionary(CmdRequest &request, CmdResponse &response)
{
...
void DjvuBridge::processPageDictionaryCheck(CmdRequest &request, CmdResponse &response)
{
