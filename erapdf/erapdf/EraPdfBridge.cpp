...
void MuPdfBridge::responseAddString(CmdResponse& response, std::wstring str16)
{
    std::string str8 = wstringToString(str16);
...
void MuPdfBridge::process(CmdRequest &request, CmdResponse &response)
{
    ...
    switch (request.cmd)
    {
        ...
        case CMD_REQ_RTL_TEXT:
            ...
            break;
        case CMD_REQ_PAGE_DICT_CHECK:
            processPageDictionaryCheck(request, response);
            break;
        case CMD_REQ_PAGE_DICT_REGISTER:
            processRegisterDictionary(request, response);
            break;
        case CMD_REQ_TTS_SENTENCE:
            processGetTTSSentence(request, response);
            break;
        case CMD_REQ_TTS_XPATH_SENTENCE:
            processGetTTSXpathSentence(request, response);
            break;
        case CMD_REQ_TTS_HITBOXES:
            processGetTTSHitboxes(request, response);
            break;
        case CMD_REQ_CALCULATE_COLONTITLES:
            processSmartColontitles(request, response);
            break;
        case CMD_REQ_INGEST_COLONTITLES:
            processRegisterColontitles(request, response);
            break;
        case CMD_REQ_CALCULATE_COLONTITLES_TEXT:
            processSmartColontitlesText(request, response);
            break;
        default:
...
{
    for (int i = 0; i < hitboxesCache.size(); i++)
    {
        hitboxesCache.at(i).reset();
    }
    hitboxesCache.clear();
...
void MuPdfBridge::analyzePageForDarkMode(fz_context *ctx, int index, int w, int h)
{
    ...
    fz_try(ctx)
    {
        fz_run_fake_display_list(ctx,pageLists[index], &ctm, &area, nullptr,index);
    }
    fz_catch(ctx)
    {
        const char* msg = fz_caught_message(ctx);
...
void MuPdfBridge::processPageText(CmdRequest& request, CmdResponse& response)
{
    ...
    std::vector<Hitbox_st> pagetext = processTextToArray( (int) pageNo);
    for (int i = 0; i < pagetext.size(); i++)
    {
        Hitbox_st curr = pagetext.at(i);
...
std::vector<Hitbox_st> MuPdfBridge::GetHitboxesBetweenXpaths(int page,
        std::string xpStart, std::string xpEnd, int startPage, int version)
{
    std::vector<Hitbox_st> result;
    std::vector<Hitbox_st> hitboxes = processTextToArray(page, version);
    if(hitboxes.empty())
    {
        return result;
    }
...
void MuPdfBridge::processPageRangeText(CmdRequest &request, CmdResponse &response) {
    ...
    std::vector<Hitbox_st> BookmarkHitboxes = GetHitboxesBetweenXpaths(page, startstr, endstr,
            startPage, version);
    ...
    {
        Hitbox_st currHitbox = BookmarkHitboxes.at(i);
        ...
        const fz_matrix_s* ctm)
{
    ...
    fz_try(ctx) {
        ...
        textCache.addItem(index, std::wstring(ctx->pageTextBuffer), PAGE_CACHE_MODE_SUPERFAST);
        memset(ctx->pageTextBuffer,0,TEXTBUFSIZE);
        res = true;
    } fz_catch(ctx) {
        const char* msg = fz_caught_message(ctx);
        LE("%s", msg);
...
void MuPdfBridge::processSmartColontitles(CmdRequest& request, CmdResponse& response)
{
    response.cmd = CMD_RES_CALCULATE_COLONTITLES;
    if (document == nullptr || pages == nullptr) {
        LE("Document not yet opened");
        response.result = RES_ILLEGAL_STATE;
        return;
    ...
    float lambda = origin_h / origin_w;
    int smart_S = colontitle_w * colontitle_h;
    ...
    //LE("MuPdfBridge::processSmartColontitles START page = [%d] [%d x %d] ", page_index, new_w, new_h);
    calcBitmapColontitleBounds(smart_crop, pixels, new_w, new_h, slice_l, slice_t, slice_r, slice_b);
    response.addFloatArray(4, smart_crop, true);
    //LE("MuPdfBridge::processSmartColontitles END   page = [%d] t = [%f] b = [%f]", page_index, smart_crop[1], smart_crop[3]);
    if (pixels) { free(pixels); }
}
...
void MuPdfBridge::processRegisterColontitles(CmdRequest &request, CmdResponse &response)
{
    response.cmd = CMD_RES_INGEST_COLONTITLES;
    if (document == nullptr || pages == nullptr)
    {
        LE("Document not yet opened");
        response.result = RES_ILLEGAL_STATE;
    ...
    colontitles_map[page_index] = std::make_pair(slice_t,slice_b);
}