...
void toResult(std::vector<Hitbox_st> &result, fz_rect &bounds, fz_irect *rr, const char *str, int len)
{
    ...
    Hitbox_st res = Hitbox_st(left, right, top, bottom, wrestext, xpath);
    result.push_back(res);
}

void toResultParaend(std::vector<Hitbox_st> &result, fz_rect &bounds, fz_irect *rr)
{
    float width = bounds.x1 - bounds.x0;
    float height = bounds.y1 - bounds.y0;
    float left = (rr->x0 - bounds.x0) / width;
    ...
    Hitbox_st res = Hitbox_st(left, right, top, bottom, wstr, std::string());
    result.push_back(res);
}
...
void processLineToResult_v2(std::vector<Hitbox_st> &result, fz_context *ctx, fz_rect &bounds, fz_text_line &line)
{
    ...
    for (span = line.first_span; span != nullptr; span = span->next)
    {
        ...
        if (span->text && span->len > 0)
        {
            for (int textIndex = 0; textIndex < span->len;)
            {
                ...
                hyph = (text.c == 0x002D);
                punct = UnicodeHelper::isPunct(text.c);
                isChar = !punct && !space;
                ...
                else
                {
                    ...
                    a.y0 = bbox.y0;
                    a.y1 = bbox.y1 + charheight / 8;

                    b.y0 = bbox.y0;
                    b.y1 = bbox.y1 + charheight / 8;

                    c.y0 = bbox.y0;
                    c.y1 = bbox.y1 + charheight / 8;
                    ...
                    if(text.c == 0xFB00 || text.c == 0xFB01 || text.c == 0xFB02 || text.c == 0xFB05 || text.c == 0xFB06 )
                    {
                        a.x0 = bbox.x0;
                        a.x1 = bbox.x1 - (charwidth / 2);

                        b.x0 = bbox.x0 + (charwidth / 2);
                        b.x1 = bbox.x1;
                        ...
                        charnum++;
                        toResult(subbuf, bounds, &a, a_text, 1);
                        charnum++;
                        toResult(subbuf, bounds, &b, b_text, 1);

                        a_text[0] = 0;
                        b_text[0] = 0;
                    }
                    else if ( text.c == 0xFB03 || text.c == 0xFB04 )
                    {
                        a.x0 = bbox.x0;
                        a.x1 = bbox.x1 - ((charwidth / 3) * 2) ;

                        b.x0 = bbox.x0 + (charwidth / 3);
                        b.x1 = bbox.x1 - (charwidth / 3);

                        c.x0 = bbox.x0 + ((charwidth / 3) * 2);
                        c.x1 = bbox.x1;
                        ...
                        toResult(subbuf, bounds, &b, b_text, 1);
                        charnum++;
                        toResult(subbuf, bounds, &c, c_text, 1);
...
void processLineToResult_v1(std::vector<Hitbox_st> &result, fz_context *ctx, fz_rect &bounds, fz_text_line &line)
{
...
void processLineToResult_v0(std::vector<Hitbox_st> &result, fz_context *ctx, fz_rect &bounds, fz_text_line &line)
{
    int index = 0;
    fz_rect rr = fz_empty_rect;
    fz_irect box = fz_empty_irect;
...
void processLineToResult(std::vector<Hitbox_st> &result, fz_context *ctx, fz_rect &bounds, fz_text_line &line, int version)
{
    switch (version)
    {
        case 0:
...
std::string getLineText_fast(fz_text_line& line)
{
    //based off processLineToResult_v2
    std::string result;
    if (line.first_span == nullptr)
    {
        return result;
    ...
    for (span = line.first_span; span != nullptr; span = span->next)
    {
        ...
        for (int textIndex = 0; textIndex < span->len; textIndex++)
        {
            fz_text_char &text = span->text[textIndex];
            switch (text.c)
            {
                ...
                default:
                    length = fz_runetochar(utf8, text.c);
                    std::string ch = std::string(utf8, length);
                    //LD("PdfText: getLineText_fast: char processing: %d [0x%04X] [%lc]",textIndex, ch.front(), text.c );
                    result.append(ch);
                    break;
            }
        }
        if(!result.empty() && !UnicodeHelper::isSpace(result.back()))
        {
            result.append(" ");
        }
    }
...
std::string getLineText(fz_context *ctx, fz_rect& bounds, fz_text_line& line)
{
    ...
    std::vector<Hitbox_st> hitboxes;
    processLineToResult(hitboxes, ctx, bounds, line,-1);
    for (int i = 0; i < hitboxes.size(); i++)
    {
        Hitbox_st curr = hitboxes.at(i);
...
std::string MuPdfBridge::getPageText(int pageNo, bool fast)
{
    if (pageNo == precise_cachedPageNum)
    {
        return precise_cachedPageText;
    }
        ...
        if (pagetext->blocks && pagetext->len > 0)
        {
            ...
            for (blockIndex = 0; blockIndex < pagetext->len; blockIndex++)
            {
                ...
                for (lineIndex = 0; lineIndex < block.u.text->len; lineIndex++)
                {
                    ...
                    LDD(LOG, "PdfText: processText: %s line processing: %d", (fast) ? "fast":"slow" , lineIndex);
                    if(fast)
                    {
                        result.append(getLineText_fast(line));
                    }
                    else
                    {
    ...
    if(!fast)
    {
        precise_cachedPageNum = pageNo;
        precise_cachedPageText = result;
    }
    return result;
}
...
void postProcessLine(std::vector<Hitbox_st> result, fz_context *ctx, fz_rect bounds, fz_page_block &block,int lineIndex, int version)
{
    if(result.size()==0 || version == 0 || version == 1)
    {return;}
    Hitbox_st curr = result.at(result.size() - 1);
...
void postProcessBlock(std::vector<Hitbox_st> result, fz_context *ctx, fz_text_page *pagetext, int blockIndex, fz_rect bounds)
{
...
std::vector<Hitbox_st> MuPdfBridge::processTextToArray(int pageNo, int version)
{
    if (!hitboxesCache.empty() && (version == -1 || version == CURRENT_MAX_VERSION) )
    {
        for (int i = hitboxesCache.size() - 1; i >= 0; i--)
        {
            if (hitboxesCache.at(i).getPage() == pageNo)
            {
                return hitboxesCache.at(i).getArray();
            }
        }
        if (hitboxesCache.size() >= 5)
        {
            hitboxesCache.erase(hitboxesCache.begin());
        }
    }

    std::vector<Hitbox_st> boxes = processTextToArray_imp(pageNo, version);
    if(version == -1 || version == CURRENT_MAX_VERSION )
    {
        hitboxesCache.emplace_back(pageNo, boxes);
    }
    return boxes;
}

std::vector<Hitbox_st> MuPdfBridge::processTextToArray_imp(int pageNo, int version)
{
    pagenum = pageNo;
...
void MuPdfBridge::processRtlText(CmdRequest &request, CmdResponse &response)
{
    ...
    std::vector<Hitbox_st> selection = processTextToArray(page);

    std::wstring respstr;
    for (int i = startindex; i <= endindex && i < selection.size(); i++)
    {
        Hitbox_st currHitbox = selection.at(i);
