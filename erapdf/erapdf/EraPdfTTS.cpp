...
std::vector<std::vector<Hitbox_st>> MuPdfBridge::getPageSentences(int page, std::vector<HitboxSentence> *curr_sentences)
{
    ...
    *curr_sentences = performAllSplittings(curr_hbs_vec, *curr_sentences);
    ...
    if(page > 0)
    {
        ...
        prev_sentences = performAllSplittings(prev_hbs_vec, prev_sentences);
        ...
        if(!prev_sentences.empty())
        {
            ...
            //LE("curr page [%d] first sentence  [%ls]",page, hitbox_stVectorToString(result.front()).c_str());
    ...
    if(curr_last && page + 1 < this->pageCount)
    {
        ...
        if(!next.empty())
        {
            ...
            //LE("curr page [%d] last sentence  [%ls]",page, hitbox_stVectorToString(result.back()).c_str());
            ...
            if(post_first && !postSentence_hbs.empty())
            {
                ...
                //LE("post_height = %f , curr_height = %f, diff = %f < %f",post_height, curr_height, fabs(post_height - curr_height), height_threshold);
...
void MuPdfBridge::processGetTTSSentence(CmdRequest &request, CmdResponse &response)
{
    response.cmd = CMD_RES_TTS_SENTENCE;
    if (document == nullptr)
    {
        LE("processGetTTSSentence: Document not yet opened");
    ...
    for( int i = 0; i < sentences.size(); i++)
    {
        ...
        //LE("EraPdf processGetTTSSentence [%d] type [%d] [%d] - [%d] text [%s]", page, sentence.type, sentence.start, sentence.end, text.c_str());
        //LE("EraPdf processGetTTSSentence [%d]       start xp [%s]", page, xp_start.c_str());
        //LE("EraPdf processGetTTSSentence [%d]       end   xp [%s]", page, xp_end.c_str());
        //LE("EraPdf processGetTTSSentence [%d]  x = [%f], y = [%f]", page, x, y);
...
void MuPdfBridge::processGetTTSXpathSentence(CmdRequest &request, CmdResponse &response)
{
    ...
    //LE("processGetTTSXpathSentence page [%d] xp [%s]",page,xpath.c_str());
    ...
    for (int i = 0; i < sentences_list.size(); i++)
    {
        ...
        if(s.contains(selected_index))
        {
            ...
            LE("found at = %d [%ls]",i,hitbox_stVectorToString(sentence_vec).c_str());
            break;
        }
    }
    ...
    //LE("EraPdf processGetTTSXpathSentence [%d] type [%d] text [%s]", page, sentence.type , text.c_str());
    //LE("EraPdf processGetTTSXpathSentence [%d]       start xp [%s]", page, xp_start.c_str());
    //LE("EraPdf processGetTTSXpathSentence [%d]       end   xp [%s]", page, xp_end.c_str());
    //LE("EraPdf processGetTTSXpathSentence [%d]  x = [%f], y = [%f]", page, x, y);
...
void MuPdfBridge::processGetTTSHitboxes(CmdRequest &request, CmdResponse &response)
{
...
void MuPdfBridge::processSmartColontitlesText(CmdRequest& request, CmdResponse& response)
{
