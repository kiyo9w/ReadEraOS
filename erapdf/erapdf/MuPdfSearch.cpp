...
{
    std::vector<Hitbox_st> hitboxes = processTextToArray(page);
    return GetXpathFromPageById(hitboxes, id, addcoords, reverse);
}

std::string MuPdfBridge::GetXpathFromPageById(std::vector<Hitbox_st> hitboxes, int id, bool addcoords, bool reverse)
{
...
std::vector<std::string> MuPdfBridge::GetXpathFromPageById(std::vector<Hitbox_st> hitboxes, std::vector<int> pos_arr, bool addcoords, int qlen)
{
...
std::vector<SearchResult> MuPdfBridge::FindAndTrim(std::wstring query, int page, int &end)
{
    ...
    for (int i = 0; i < xp_arr.size(); i++)
    {
        ...
        {
            std::wstring str;
            std::vector<Hitbox_st> subset(&hitboxes[window.start_], &hitboxes[window.end_]);
...
std::vector<Hitbox_st> MuPdfBridge::GetSearchHitboxes(std::vector<Hitbox_st> hitboxes, int page, std::wstring query, bool wordBeginFlag)
{
    ...
    if(cachedHitboxesPageNum != page)
    {
        cachedHitboxesPageText.clear();
        for (auto & hb : hitboxes)
        {
            cachedHitboxesPageText += hb.text_;
        }
        cachedHitboxesPageText = lowercase(cachedHitboxesPageText);
        cachedHitboxesPageNum = page;
    }

    int qlen = query.length();

    bool shortQuery = qlen < 3;
    bool query_C_or_J = UnicodeHelper::checkCJ(query);
    bool query_K = UnicodeHelper::checkK(query);

    int startpos = 0;
    int pos = 0;
    while (pos < cachedHitboxesPageText.size())
    {
        pos = pos_f(cachedHitboxesPageText, query, startpos);
        if (pos == -1)
        {
            break;
        }
        ...
        {
            if(shortQuery && !query_K && pos + qlen < cachedHitboxesPageText.size())
            {
                int ch_after = cachedHitboxesPageText.at( pos + qlen );
            ...
            int ch_before = cachedHitboxesPageText.at(pos-1);
            if (!UnicodeHelper::isPunct(ch_before) && !UnicodeHelper::isSpace(ch_before))
            {
                startpos = pos + qlen;
                continue;
            }
        ...
        for (int i = pos; i < pos + qlen && i < hitboxes.size(); i++)
        {
            Hitbox_st hb = hitboxes.at(i);
            hb.text_ = xp;
...
std::vector<Hitbox_st> MuPdfBridge::SearchForTextHitboxes(int page, std::wstring query, bool wordBeginFlag)
{
    std::vector<Hitbox_st> result;
    ...
    std::vector<Hitbox_st> base = processTextToArray(page);
    ...
    if (page != 0)
    {
        if (checkBeforePrevPage(base, query))
        {
            ...
            if (last_len > 0)
            {
                std::vector<Hitbox_st> first = GetSearchHitboxesPrevPage(base, query, last_len);
    ...
    */
    //LE("SearchForTextHitboxes query = [%ls]",query.c_str());

    std::vector<Hitbox_st> main = GetSearchHitboxes(base, page, query, wordBeginFlag);
    ...
    if (page < pageCount-2)
    {
        std::vector<Hitbox_st> last = GetSearchHitboxesNextPage(base, page, query);
    ...
    */
    //LE("SearchForTextHitboxes result length = %d",result.size());
    if (result.empty())
    {
        return result;
    }
...
std::vector<std::wstring> MuPdfBridge::checkIfPageHasWords(uint32_t pagenum)
{
    LE("checkIfPageHasWords [%d]",pagenum);
    std::wstring wtext = textCache.findPageText(pagenum);;

    if (!wtext.empty())
    {
        LI("Cached text");
        wtext = ReplaceUnusualSpaces(wtext);
        wtext = lowercase(wtext);
    }
    else
    {
        LI("Generating text in fast mode");
        std::string pageText = getPageText(pagenum, true);
        if (pageText.empty())
        {
            return std::vector<std::wstring>();
        }
        wtext = stringToWstring(pageText);
        wtext = ReplaceUnusualSpaces(wtext);
        wtext = lowercase(wtext);

        int mode = (pagenum == precise_cachedPageNum) ? PAGE_CACHE_MODE_DEFAULT : PAGE_CACHE_MODE_FAST;
        textCache.addItem(pagenum, wtext, mode);
    }

    //LE("checkTextForDict [%ls]",wtext.c_str())
    std::vector<std::wstring> result = checkTextForDict(wtext, searchDictionary);
    if(!searchDictionaryCJK.empty())
    {
        if(UnicodeHelper::checkCJ(wtext))
        {
            std::vector<std::wstring> cjk_result = checkTextForDictSimple(wtext, searchDictionaryCJK);
            result.insert(end(result), begin(cjk_result), end(cjk_result));
        }
    }
    if(!searchDictionaryRTL.empty())
    {
        if(UnicodeHelper::checkRTL(wtext))
        {
            std::vector<std::wstring> rtl_result = checkTextForDict(wtext, searchDictionaryRTL);
...
void MuPdfBridge::processRegisterDictionary(CmdRequest &request, CmdResponse &response)
{
    ...
    std::string temp(val);
    std::wstring query_string = stringToWstring(temp);
    query_string = lowercase(query_string);

    searchDictionary.clear();
    searchDictionaryCJK.clear();
    searchDictionaryRTL.clear();
    std::vector<std::wstring> origs = split(query_string, '|');

    for (const auto& word : origs)
    {
        if(UnicodeHelper::checkCJ(word) )
        {
            searchDictionaryCJK.emplace_back(word);
        }
        else if (UnicodeHelper::checkRTL(word))
        {
            searchDictionaryRTL.emplace_back(word,replaceDelimWithSpace(word,DELIM));
        }
        else
        {
            searchDictionary.emplace_back(word,replaceDelimWithSpace(word,DELIM));
        }
    }
...
void MuPdfBridge::processPageDictionaryCheck(CmdRequest &request, CmdResponse &response)
{
    ...
    std::vector<std::wstring> result = checkIfPageHasWords(pagenum);

    std::wstring result_str;
    for (auto & word : result)
    {
        //LE("word = [%ls]",word.c_str());
        result_str.append(word + L'|');
    }
    if(result_str.back() == L'|')
    {
        result_str = result_str.substr(0,result_str.length()-1);
    }
    LE("processPageDictionaryCheck result_str = [%ls]",result_str.c_str());
    responseAddString(response, result_str);
}
...
void MuPdfBridge::processTextSearchHitboxes(CmdRequest &request, CmdResponse &response)
{
    ...
    int page = atoi(keys.at(0).c_str());
    bool wordBeginFlag = (atoi(keys.at(1).c_str()) == 1);
    std::string query_s  = keys.at(2);
    ...
    {
        query = lowercase(query);
        std::vector<Hitbox_st> hitboxes = SearchForTextHitboxes(page, query, wordBeginFlag);

        for (int i = 0; i < hitboxes.size(); i++)
        {
            Hitbox_st currHitbox = hitboxes.at(i);
...
{
    SearchResult result;
    std::vector<Hitbox_st> base1 = processTextToArray(page);
    ...
    std::vector<Hitbox_st> base2 = processTextToArray(page+1);

    int qlen2 = qlen - qlen1;

    std::wstring q2 = query.substr(qlen1,qlen2);

    std::vector<Hitbox_st> base2sub(&base2[0], &base2[qlen2]);
    std::wstring text2;
    ...
    if (pos2 != -1)
    {
        base2 = std::vector<Hitbox_st>(&base2[0], &base2[pos2]);
    }

    std::vector<Hitbox_st> globaltext;
    for (int i = 0; i < base1.size(); i++)
    {
    ...
    if (window.contains(curr_pos))
    {
        ...
        std::vector<Hitbox_st> prev_arr(&globaltext[window.start_], &globaltext[window.end_]);
        std::wstring str;
...
std::vector<Hitbox_st> GetSearchHitboxesPrevPage(std::vector<Hitbox_st> base, std::wstring query, int last_len)
{
    ...
    std::vector<Hitbox_st> text1(&base[0], &base[qlen]);
...
std::vector<Hitbox_st> GetSearchHitboxesNextPage(std::vector<Hitbox_st> base1, int page, std::wstring query)
{
    std::vector<Hitbox_st> result;
    if (base1.empty())
    {
