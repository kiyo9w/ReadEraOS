...
void fz_run_display_list(fz_context *ctx, fz_display_list *list, fz_device *dev, const fz_matrix *top_ctm,
                         const fz_rect *scissor, fz_cookie *cookie, int pageindex)
{
    int objcounter = 0;
        ...
        fz_try(ctx)
                {
                    switch (n.cmd)
                    {
                        ...
                        case FZ_CMD_FILL_TEXT:
                            ...
                            fz_text* txt = *(fz_text **) node;

                            //static allocated array works, while malloc-ed one crashes with segfault
                            //BUT IT ALSO CRASHES ON RELEASE
                            //BUT STATIC ALSO CRASHES
                            //So we allocated buffer in CTX struct object to avoid the allocations
                            unsigned int j = wcslen(ctx->pageTextBuffer);
                            for (int i = 0 ; i < txt->len && j < TEXTBUFSIZE ; i++) // MAXLEN - 1 is to save some space for space between nodes
                            {
                                //LE("ch = [%lc]", txt->items[i].ucs);
                                int ucs = txt->items[i].ucs;
                                switch (ucs)
                                {
                                    case 0xFB00: // ff
                                        ctx->pageTextBuffer[j++] = 'f';
                                        ctx->pageTextBuffer[j++] = 'f';
                                        break;
                                    case 0xFB01: // fi
                                        ctx->pageTextBuffer[j++] = 'f';
                                        ctx->pageTextBuffer[j++] = 'i';
                                        break;
                                    case 0xFB02: // fl
                                        ctx->pageTextBuffer[j++] = 'f';
                                        ctx->pageTextBuffer[j++] = 'l';
                                        break;
                                    case 0xFB05: // long st
                                    case 0xFB06: // st
                                        ctx->pageTextBuffer[j++] = 's';
                                        ctx->pageTextBuffer[j++] = 't';
                                        break;
                                    case 0xFB03: // ffi
                                        ctx->pageTextBuffer[j++] = 'f';
                                        ctx->pageTextBuffer[j++] = 'f';
                                    ...
                                    case 0xFB04: // ffl
                                        ...
                                        break;
                                    default:
                                        ctx->pageTextBuffer[j++] = ucs;
                                        break;
                                }
                            }
                            ctx->pageTextBuffer[j] = L' ';
                            //LE("ctx->pageTextBuffer = [%ls]", ctx->pageTextBuffer);
...
int count_analyze_objects(fz_context *ctx, fz_display_list *list, const fz_matrix *top_ctm, const fz_rect *scissor, fz_cookie *cookie)
{
    int res = 0;
...
void fz_run_fake_display_list(fz_context *ctx, fz_display_list *list, const fz_matrix *top_ctm, const fz_rect *scissor, fz_cookie *cookie, int pageindex)
{
    ctx->darkmode_objs[pageindex].pagenum = pageindex;
    ctx->darkmode_objs[pageindex].objcount = 0;
    int objnum = count_analyze_objects(ctx, list, top_ctm, scissor, cookie);
                    ...
                    switch (n.cmd)
                    {
                        ...
                        case FZ_CMD_FILL_IMAGE:
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    //LE("Convert to rgb");
                                    fz_pixmap *check = fz_new_pixmap(ctx, fz_device_rgb(ctx), pixmap->w, pixmap->h);
                                    fz_convert_pixmap(ctx, check, pixmap);
                                    invert = fz_count_pixmap_needs_invert(ctx, check);
                                    fz_drop_pixmap(ctx,check);
                                }
                                else
                                {
                                    //LE("already rgb");
                                    invert = fz_count_pixmap_needs_invert(ctx, pixmap);
                                }
                            }
